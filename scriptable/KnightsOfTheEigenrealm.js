...
const Dungeon = {
  ...
  nextRoom(){
    this.run.depth++;
    this.scried=null;                 // a reading was about this room; it is now behind you
  ...
  foresightBlock(){
    if(this.scried)
      return \`<div class="panel" style="border-color:#6a44b0">
        <div class="row" style="display:flex;gap:8px;align-items:center">
          <span style="font-size:22px">🔭</span>
          <div class="small" style="flex:1;line-height:1.5">\${this.scried.line}</div>
        </div></div>\`;
    const carried = Loadout.chosen().some(a=>a.id==='farsight');
    if(!carried) return '';
    const left = this.forkCharges('farsight');
    if(left<1) return \`<div class="center small" style="opacity:.6;margin:4px 0">
        🔭 Farsight spent for this descent</div>\`;
    return \`<button class="btn" style="border-color:#6a44b0"
        onclick="Dungeon.scry()">🔭 Scry the passage below
