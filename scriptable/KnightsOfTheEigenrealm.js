...
.rp{display:inline-block; width:12px; height:12px; border-radius:50%; border:2px solid var(--edge); background:#171129;}
.rp.done{background:var(--green); border-color:var(--green);}
.rp.miss{background:var(--red); border-color:var(--red);}
.rp.now{border-color:var(--gold); box-shadow:0 0 8px var(--gold);}
...
/* ---- perfect strike ---- */
/* ---- being shown the working ---- */
#showwork{margin-top:8px;}
#showwork .wbtn{display:block; width:100%; padding:10px; border-radius:12px; cursor:pointer;
  text-align:center; font-size:13.5px; color:var(--dim);
  border:1px dashed var(--line); background:transparent;}
#showwork .wbtn:active{background:rgba(255,255,255,.04);}
.wkbox{margin-top:4px;}
.wkrule{border-left:2px solid var(--gold); padding:4px 0 4px 10px; margin-bottom:10px;
  font-size:13.5px; color:var(--dim);}
.wksteps{margin:0 0 4px; padding-left:22px;}
.wksteps li{margin-bottom:9px; line-height:1.55;}
.wksteps li::marker{color:var(--gold); font-weight:600;}
.wkask{color:var(--dim); font-size:13px;}
.wkgot{color:var(--ink); font-weight:600;}
.wkwhy{font-size:14px;}
.wkans{margin-top:10px; padding-top:8px; border-top:1px solid var(--line);}
.wkans b{color:var(--gold);}
...
<div id="app">
  ...
  <div class="screen" id="s-battle">
    ...
    <div class="panel" id="qpanel">
      ...
      <div id="showwork" style="display:none"></div>
    </div>
  ...
  <div class="screen" id="s-prefs">
    <div id="prefList"></div>
    <button class="btn ghost" onclick="UI.back()">← Back</button>
    <!-- Which build this is, so a bug report can name it. -->
    <div id="buildStamp" class="small center" style="opacity:.5;margin-top:14px"></div>
    <div style="height:20px"></div>
  </div>
...
const Mastery = {
  ...
  /* The write. One graded answer nudges m toward its result on an exponential
     moving average, with two deliberate asymmetries.

     The learning rate α = w/(2 + seen) shrinks as evidence piles up, so the
     first answers on a topic move the needle and later ones only confirm it.
     A plain running average would let a long-ago streak of luck dominate
     forever; an EMA forgets old evidence at a controlled rate, which is what
     "how well do you know this now" has to mean. It is also two numbers rather
     than a history, which is why a topic costs 22 bits in a save and not a log.

     And a miss pulls all the way to zero while a hit only pulls to \`quality\`,
     because forgetting is cheaper than learning.

     \`weight\` scales how much an outcome counts. A mote is one small step of a
     topic, so four of them must not count as four full answers — otherwise the
     scheduler believes you have mastered what you were merely walked through. */
  update(key, ok, weight, quality){
    const r=this.rec(key);
    const was=r.m;
    const w = weight===undefined ? 1 : weight;
    const target = ok ? clamp(quality===undefined?1:quality, 0, 1) : 0;
    // α is read off the evidence BEFORE this answer joins it.
    const alpha = clamp(w/(2 + (r.seen||0)), 0, 1);
    r.seen += w;
    if(ok) r.c+=w; else r.w+=w;
    r.m = clamp(r.m + alpha*(target - r.m), 0, 1);
  ...
  /* The fade. What is stored never moves while you are away; what the game
     READS is an effective mastery that decays with time since you last saw the
     topic. Nothing new is written to the save — the fade is derived at read
     time from the two fields already there.

     Retention is exp(−days / strength), and strength is 2·seen days — it grows
     with how often you have met the topic. That single line is what makes the
     review intervals double on their own, because the point where retention
     has fallen by the same amount moves out every time seen doubles —

         seen 1 → 2 days,  2 → 4 days,  4 → 8 days,  8 → 16 days

     — so the scheduler never needs a calendar of its own. It watches effective
  ...
  retention(r){
    ...
    return Math.exp(-d / this.strength(r));
  },
  // Effective mastery: what every reader below should ask for, rather than the
  // raw stored number, which only says how well you knew it the day you left.
  eff(key){
    const r=Game.s && Game.s.topicStats[key];
    if(!r || !r.seen) return 0;
    return clamp((r.m||0) * this.retention(r), 0, 1);
  },
  ...
  /* The pick. Every candidate is scored by urgency, and the room takes a
     weighted sample rather than the argmax — always serving the single most
     urgent topic is predictable and reads as nagging, while a sample keeps the
     variety that makes a run feel like a run.

     Three signals, and their weights are the real tuning knobs: whether this
     game feels like drilling weaknesses, honouring the schedule, or staying in
     theme.
       gap        how far effective mastery is from solid — drill the weak
       staleness  how far past its review point the topic has drifted
       relevance  whether this room's setting is about this topic

     Unseen topics carry a deliberate premium so breadth gets introduced, and
     the topic just asked is excluded outright: interleaving is most of why
     spacing works, and asking the same thing twice running undoes it. */
  W_GAP:3.0, W_STALE:1.6, W_RELEVANCE:1.2, W_FLOOR:0.25, W_UNSEEN:5,
  _last:null,
  urgency(key, prefer){
    const r=Game.s && Game.s.topicStats[key];
    if(!r || !r.seen) return this.W_UNSEEN;                  // unseen: introduce it
    const gap  = 1 - this.eff(key);                          // 0 solid … 1 forgotten
    const byQ  = Math.min((Game.s.qCount - r.last)/this.dueIn(r), 2.5);
    const byDay= Math.min(this.daysSince(r)/this.dueDays(r), 2.5);
    const stale= Math.max(byQ, byDay);
    const rel  = (prefer && prefer.length) ? (prefer.indexOf(key)>=0 ? 1 : 0) : 0;
    return this.W_FLOOR
         + this.W_GAP*gap*gap                                // weak topics dominate
         + this.W_STALE*stale
         + this.W_RELEVANCE*rel;
  },
  // Kept for callers that want the raw score of one topic.
  weight(key){ return this.urgency(key); },
  /* \`pure\` is for callers building under a seed — a dungeon room, which has to
     rebuild identically when a saved run is resumed. The no-repeat rule is
     session state that a fresh page does not have, so consulting it there would
     make a resumed room draw a different topic than the one you walked out of. */
  pick(pool, prefer, pure){
    if(!pool || !pool.length) return null;
    if(!Game.s) return R.pick(pool);
    // Never ask the same topic twice running, unless it is the only one there is.
    let cand = (!pure && pool.length>1) ? pool.filter(k=>k!==this._last) : pool.slice();
    if(!cand.length) cand = pool.slice();
    const ws=cand.map(k=>this.urgency(k, prefer));
    const total=ws.reduce((a,b)=>a+b,0);
    let hit=cand[cand.length-1];
    if(total>0){
      let t=total*R._r();
      for(let i=0;i<cand.length;i++){ t-=ws[i]; if(t<=0){ hit=cand[i]; break; } }
    } else hit=R.pick(cand);
    if(!pure) this._last=hit;
    return hit;
  },

  /* Difficulty, read off the same number. Ease off where a player is
     struggling and push where they are solid — and because this reads
     EFFECTIVE mastery, a topic you have not touched in a fortnight comes back
     gently instead of at the level you left it. */
  /* \`base\` may arrive above three — the Library's stacks add to it — so it is
     clamped on the way in as well as on the way out. Without that, a shelf deep
     enough would ask for a tier that has no questions in it. */
  adjustDiff(key, base){
    base = clamp(base, 1, 3);
    const r=Game.s&&Game.s.topicStats[key];

    if(!r || r.seen<3) return Math.min(base,2);          // go gently on new material
    const m=this.eff(key);
    if(m < 0.45) return 1;
    if(m > 0.85) return clamp(base+1,1,3);
    return base;
  },

  /* Teaching verbosity, read off the same number: help is 1 − mastery, in
     bands. 3 is a topic you are lost in and 0 is one you own. This is what
     decides whether the rule is free to look at and whether a hard topic
     arrives as a ladder or in one piece. */
  help(key){
    const m=this.eff(key);
    return m>=0.85 ? 0 : m>=0.6 ? 1 : m>=0.35 ? 2 : 3;
  },
  // Is this a weak topic being deliberately brought back?
  isReview(key){
    const r=Game.s&&Game.s.topicStats[key];
    return !!(r && r.seen>=3 && this.eff(key)<0.6);
  },
...
/* The Scriptorium's bench, and the only thing in the game that spends a
   material on KNOWLEDGE rather than on kit.

   Pages buy recency, and nothing else. Binding a topic sets its clock back —
   the game treats it as freshly practised, so its effective mastery stops
   decaying for a while — and does not move the mastery itself by a hair. That
   line is the whole ethic of the thing: writing something down is how you stop
   forgetting it, not how you learn it, and a currency that could buy the second
   would make every other room in the game optional.

   \`each\` binds one topic; the bench renders the choice. */
const SCRIPTORIUM = [
  {id:'s_one', nm:'Bind a single leaf', ic:'📄', page:18, topics:1,
   ds:'Sets one topic’s clock back — the one you have let go coldest.'},
  {id:'s_few', nm:'Bind a quire',        ic:'📗', page:44, topics:4,
   ds:'The four coldest, at a better rate a leaf.'},
  {id:'s_all', nm:'Bind the whole strand', ic:'📚', page:110, topics:12,
   ds:'A dozen at once. Nothing here touches what you know — only how long ago you knew it.'}
];

/* The Apothecary's bench. Herbs are gathered in the Wilds and spent only here,

   which is what closes the second material loop: the Deep gives up ore and
   essence, the Wilds gives up herbs, and the Keep turns all three into things
   you take back out with you.

   It differs from the other two benches in shape as well as in currency. The
   Forge makes one-offs and the Rune Bench sells permanents; this one mostly
   sells REPEATABLE bundles, because what the Wilds actually costs a knight is
   consumables — you walk a long way out there and everything you drink on the
   way is gone. The one permanent on the list is the reason to keep gathering
   after the bundles stop being interesting.

   \`each\` is what one purchase hands over; \`keep\` marks the permanent. */
const APOTHECARY = [
  {id:'a_draught', nm:'A course of draughts', ic:'🧪', herb:14,
   ds:'Three Healing Draughts, steeped from what you gathered.',
   each(g){ g.items.potion += 3; }},
  {id:'a_insight', nm:'Clarity of sight',   ic:'🔮', herb:12,
   ds:"Two Sage's Insights. Gold buys these too — herbs buy them while you are still out.",
   each(g){ g.items.insight += 2; }},
  {id:'a_feather', nm:'A phoenix feather',  ic:'🪶', herb:46,
   ds:'One Feather. Dear, and the only thing here that decides whether a walk ends.',
   each(g){ g.items.feather += 1; }},
  {id:'a_satchel', nm:"The herbalist's satchel", ic:'🎒', herb:60, keep:true,
   ds:'Every draught you drink from now on restores half again as much. Gold cannot buy this.',
   each(g){ g.brewed.satchel = 1; },
   owned(g){ return !!(g.brewed && g.brewed.satchel); }}
];
...
/* The four materials, as the Crucible sees them: interchangeable in principle
   and gathered in four different places. Ordered by where they come from, so
   the row on the bench reads like a tour of the map. */
const MATERIALS = [
  {id:'ore',     nm:'ore',     ic:'🪨', from:'seams in the Deep'},
  {id:'essence', nm:'essence', ic:'🜄', from:'chests, and the Sanctum'},
  {id:'herb',    nm:'herbs',   ic:'🌿', from:'thickets in the Wilds'},
  {id:'page',    nm:'pages',   ic:'📄', from:'passages in the Library'}
];
...
const Codec = {
  ...
  encode(k, g, now){
    ...
    // v5: what the Wilds gave up, and the one thing the Apothecary makes that
    // is kept rather than drunk. Appended after the settings block for the same
    // reason everything else was appended: a v4 code simply ends above here and
    // still reads in full.
    o.varint(Math.max(0, Math.round(mt.herb||0)));
    o.bits((g.brewed && g.brewed.satchel) ? 1 : 0, 1);

    // v6: what the Library gave up. Appended after v5 for the same reason
  ...
  decode(str){
    ...
    g.mats={ore:0, essence:0, herb:0, page:0};
    g.runes={};
    ...
    // v5 and later carry the herb pouch and the satchel. A knight from an older
    // code has simply never been out to the Wilds.
    g.brewed={};
    if(ver>=5){
      g.mats.herb=b.varint();
      if(b.bits(1)) g.brewed.satchel=1;
    }
...
const Game = {
  ...
  fresh(){
    return {
      ...
      firstRun:0,          // has this knight finished the cellar? gates the map
      cheated:0,           // were the gates opened by the testing door? (see Cheats)
      ...
      shown:0,                                  // times the working was asked for, never mastery
  ...
  migrate(){
    ...
    if(!this.s.metRoom) this.s.metRoom={};
    // Anyone already playing has long since earned the map and does not get
    // sent to the cellar or read an opening they have been past for weeks.
    if(typeof this.s.firstRun!=='number'){
      const played = this.s.lvl>1 || Object.keys(this.s.cleared||{}).length
                  || (this.s.stats && this.s.stats.correct);
      this.s.firstRun = played ? 1 : 0;
      if(played){ this.s.seenOpening=1; this.s.metRoom.monster=1; }
    }
    ...
    if(!this.s.mats) this.s.mats={ore:0, essence:0, herb:0};
    ...
    if(typeof this.s.shown!=='number') this.s.shown=0;
  ...
  start(){
    ...
    UI.refreshTitle();
    // A cold start reads the three cards and then goes straight down to the
    // cellar. No map, no shop, no lore beyond those cards — the first thing a
    // new knight does is answer a question and hit something with the answer.
    Opening.play(()=>Quartermaster.open());
  },
  rest(){
...
/* -------------------------------- battle ---------------------------------- */
/* -------------------------------- combat maths ---------------------------- */
/* The arithmetic of a blow, with nothing in it that touches the screen or the
   clock. Battle owns the fight; this owns the numbers — which means the balance
   harness can play thousands of runs against exactly the sums a player meets,
   instead of a second copy of them that drifts. Every random draw is passed in
   rather than taken here, so a simulated run is reproducible from a seed.     */
const Combat = {
  // A streak multiplies damage, capped at seven so a hot run cannot run away.
  comboMul(combo){ return 1 + Math.min(combo,7)*.15; },

  /* One answered-correctly strike. \`jitter\` is the ±10% spread on every blow,
     \`bonus\` folds in Perfect Strike, unassisted and rite-finisher multipliers. */
  strike(dmg, speed, combo, crit, rage, bonus, jitter){
    return Math.round(dmg * speed * this.comboMul(combo)
                    * (crit?2:1) * (rage?2.5:1) * (bonus||1) * jitter);
  },
  // The foe's answer to a miss. Armour subtracts, but a blow always lands.
  foeHit(atk, def, jitter){ return Math.max(4, Math.round(atk*jitter - def)); },
  /* The wind-up blow, which arrives on the foe's own clock whatever you
     answered — a correct answer braces it rather than avoiding it, which is
     why armour and draughts always matter. */
  slam(atk, def, braced){
    return Math.max(3, Math.round(atk*1.7*(braced?0.35:1) - def));
  }
};
...
const Battle = {
  ...
  // A dungeon room's fight. The shell hands in the foe and a callback that
  // takes just won/lost — the room kind turns that into the outcome the
  // Dungeon applies. Battle never touches the save here; the shell does.
  beginDungeon(foe, done){
    this.mode='dungeon';
    this.realm=Dungeon.realm; this.ri=0; this.fi=Math.min(6, Dungeon.run.depth);
    this._roomDone=done;
    this._start(foe);
  },
  _start(f){
  ...
  updateBars(){
    ...
    document.getElementById('comboTxt').textContent =
      ...
      (this.mode==='dungeon' ? \`⛏️ Depth \${Dungeon.run.depth}  ·  \` : '') +
      (this.mode==='daily' ? \`🗡️ \${Math.min(Daily.n+1,Daily.LEN)}/\${Daily.LEN}  ·  \${Daily.score} pts  ·  \` : '') +
      \`Combo ×\${Combat.comboMul(this.combo).toFixed(2)}  ·  streak \${this.combo}\`;
    ...
    if(fb) fb.textContent = this.mode==='arena' ? '🚪 Retire with your winnings'
      : this.mode==='dungeon' ? '🚪 Climb out with your haul' : '🏃 Retreat';
    this.renderPowers();
  },
  ...
  renderPowers(){
    ...
    const add=(k,label,enabled,fn)=>{
      ...
      d.dataset.pw=k;                          // what this is, for anyone reading the bar
      d.onclick=fn; bar.appendChild(d);
    };
    /* Skill abilities come first: they are what this knight KNOWS, and they
       cost nothing but the learning. An ability at zero charges still shows,
       greyed and labelled with its band — seeing "weak · 0" beside a skill is
       how a player learns that studying the strand is what fills it. */
    for(const a of Loadout.chosen()){
      if(a.where==='fork') continue;            // spent at the fork, not in here
      const left=(this.skillLeft && this.skillLeft[a.id]) || 0;
      add('skill-'+a.id, \`\${a.ic} \${a.nm} ×\${left}\`,
          left>0 && a.ready(), ()=>Battle.useSkill(a.id));
    }
    ...
    /* Carried wards are a readout rather than a button, the way the Feather
       already is: they are not pressed, they spend themselves on the next blow
       that lands. But they have to be visible, or the strongest thing a knight
       can be carrying into a fight is a number nobody can see. */
    const wards=Dungeon.wards();
    if(wards>0) add('sigil', \`🌀 Ward ×\${wards}\`, false, ()=>{});
  },

  /* A raised Ward halves the next blow that actually lands, whichever kind it
     is — an ordinary strike or a wind-up slam — and is spent doing it. */
  spendWard(dmg){
    if(!this.wardUp) return dmg;
    this.wardUp=false;
    const cut=Math.max(1, Math.round(dmg/2));
  ...
  /* A blow the foe throws for missing an answer, run past everything that might
     stop it. Three things can, and the order they are tried in is the whole of
     the design: the beast first, because it costs nothing and is simply true of
     you; then a raised Ward, because the player pressed it for this blow; then a
     banked ward-stone layer, because it will still be there for the next one.
     Cheapest first, so nothing a knight chose is ever spent on a blow something
     free was going to eat. */
  takeHit(raw){
    if(this.beastLeft){
      this.beastLeft--;
    ...
    if(this.wardUp) return this.spendWard(raw);
    return Dungeon.spendSigil(raw);
  },

  /* Spend one use of a skill ability. Charges are per fight and come from
     mastery, so this never touches the save — what it costs is knowledge you
     already had, not an item you owned. */
  useSkill(id){
    const a=Loadout.byId(id);
    if(!a || !this.skillLeft || (this.skillLeft[id]||0)<1) return;
    if(!a.ready()) return;
    this.skillLeft[id]--;
    a.go();
  ...
  usePotion(){
    ...
    // The satchel is the one thing herbs buy that gold cannot, and this is the
    // whole of it: a draught goes half again as far, everywhere, for good.
    g.items.potion--; const heal=Math.round(g.maxHp*.45*Apothecary.draughtMul());
  ...
  nextQuestion(){
    ...
    if(this.mode==='daily'){
      ...
      // Topic choice is weighted by urgency: weak, faded and overdue topics
      // surface more. In the wide pools — the Deep and the Arena draw on
      // everything you have met — the realm you are working through is passed
      // as the relevance signal, so a descent reinforces the material the
      // campaign is currently teaching instead of ranging at random.
      let pool=this.realm.pool;
      if(this.foe.boss && R.chance(.3) && this.ri>0) pool=REALMS[R.i(0,this.ri-1)].pool;
      const wide = this.mode==='arena' || this.mode==='dungeon';
      const setNow = this.mode==='dungeon' ? Dungeon.set() : null;
      const prefer = !wide ? null
                   : (settingTopics(setNow) ||
                      (REALMS[Game.s.realm] ? REALMS[Game.s.realm].pool : null));
      key=Mastery.pick(pool, prefer);
      const base = (this.mode==='arena' ? 3
                 : clamp(1 + Math.floor(this.fi/2) + (this.foe.boss?1:0), 1, 3))
                 // The Library's stacks reach into fights too — it would be a
                 // strange shelf where only the chests got older.
                 + (this.mode==='dungeon' ? Dungeon.stacks() : 0);
  ...
  /* The door out, offered on every question the game can work — which is all of
     them. Deliberately quiet: a dashed outline rather than a gold button,
     because it is the choice that costs you the turn and should not read as the
     obvious one. */
  SHOWN: Object.freeze({shown:true}),   // an object, so it can never === an answer
  renderWork(){
    const w=document.getElementById('showwork');
    const st=this.step();
  ...
  /* Routed through answer() with a sentinel rather than resolving the turn
     itself. The sentinel can never equal the answer, so the turn lands exactly
     as a miss does — streak broken, foe's turn, wind-up still winding — and
     every piece of that arithmetic stays in one place. What \`shownWork\`
     changes is only the two things that must differ: the mastery model is not
     told, and the panel shows the derivation instead of the verdict. */
  showWork(){
    if(this.answered) return;
    this.shownWork=true;
    Game.s.shown=(Game.s.shown||0)+1;
    this.answer(null, this.SHOWN);
  },
  openPad(){
    this.padVal='';
  ...
  answer(btn, choice){
    ...
    /* Being shown the working tells the mastery model nothing at all — not a
       wrong answer, not a question answered, and not a fresh timestamp. See the
       Working module: recency is what pages buy, and this must not be a bench. */
    this.lastMastery = this.shownWork ? null : Game.recordAnswer(this.cur.key, ok, weight);
    ...
    if(ok){
      ...
      if(this.steadyUp){
        this.steadyUp=false;
        Anim.float(560, H-290, 'STEADY', '#c9b7ff');
      } else {
    ...
    if(this.slamNext){
      this.slamNext=false;
      this.lastSlam=this.spendWard(Combat.slam(this.foe.atk, this.defStat(), ok));
  ...
  showExplain(ok, picked, st){
    ...
    /* Shown rather than answered: the derivation replaces the verdict. No
       "the answer was", because nothing was guessed — and no rule restated
       above it, because the working carries the rule itself. */
    if(this.shownWork){
      const w=Working.forStep(st);
      ...
      Figure.render(w.fig, document.getElementById('afig'));
      const wb=document.getElementById('contBtn');
      wb.onclick=()=>{ wb.onclick=null; wb.disabled=true; this.afterTurn(); };
      this.revealScroll(e);
      return;
    }
  ...
  afterTurn(){
    ...
    if(this.rite){
      ...
      if(r.i < r.motes.length){
        this.answered=false;
        this.shownWork=false;                    // each rung is asked afresh
      ...
      /* And the flag with it. A rung shown earlier must not follow the ladder
         out: the assembled solution is a real answer and has to reach the
         mastery model, or a single peek partway up would silently cost the
         player credit for the whole topic. */
      this.shownWork=false;
  ...
  win(){
    ...
    if(this.mode==='dungeon'){ const cb=this._roomDone; setTimeout(()=>{Anim.stop(); cb(true);},1100); return; }
    const g=Game.s;
  ...
  lose(){
    ...
    if(this.mode==='dungeon'){ const cb=this._roomDone; setTimeout(()=>{Anim.stop(); cb(false);},900); return; }
    const g=Game.s;
  ...
  flee(){
    ...
    if(this.mode==='dungeon'){ Dungeon.leave(true); return; }
    UI.go('s-map');
...
/* One card per topic: the rule in a sentence and a worked scrap. Free to peek
   ...
   enough to matter.                                                          */
const Codex = {
  /* Peeking at the rule is free while you still need the help, and costs your
     combo once you do not. Banding it on mastery rather than on a count of
     sightings means a topic you have let go cold offers its rule freely again
     — which is the honest answer to "have I been taught this yet". */
  costsCombo(key){
    const r = Game.s && Game.s.topicStats[key];
    if(!r || !r.seen) return false;                 // never charge for a first meeting
    return Mastery.help(key) <= 1;                  // steady or solid: you know this
  },
...
/* Being shown the working.
 *
 * Every other teaching in the game arrives AFTER you commit: answer, then read
 * why. That is the right order for practice and the wrong order for a player
 * who is genuinely stuck, because the only way to reach the explanation is to
 * guess — and a lucky guess banks mastery you do not have.
 *
 * So this is the honest door out: ask to be shown, and the problem is worked
 * in front of you, in steps, ending on the answer. What it costs is the strike.
 * You deal no damage, the streak breaks, the foe takes its turn, and — the part
 * that matters — the topic records NOTHING. Not a correct answer, not a wrong
 * one, and not a fresh timestamp either. Being shown is not knowing, and the
 * mastery model must never hear about it.
 *
 * That last point is load-bearing rather than fussy. Recency is what the
 * Scriptorium sells for pages; a button that quietly reset a topic's clock
 * would be a free bench, and would undercut the one currency the game is most
 * careful about.
 *
 * Where the steps come from, in order of preference:
 *
 *   1. A mote ladder. Twelve topics are already written as a chain of small
 *      questions — pick u, differentiate it, rewrite, integrate — and that IS
 *      the working, authored by hand rather than inferred.
 *   2. The explanation. Every generator writes \`ex\` as the arithmetic of THIS
 *      instance rather than a restated rule, and they are written sequentially:
 *      the rule, then the numbers, then what it means. Split on sentences, that
 *      is a genuine derivation, and all seventy split into two to five parts.
 *
 * The codex rule rides on top as a heading where the topic has one, because a
 * derivation is easier to follow when you have been told what you are doing. */
const Working = {
  /* Sentence boundaries, without breaking on the decimals, vectors and
     bracketed fragments the explanations are full of. A step has to begin with
  ...
  /* The working for one question, as data. \`steps\` is what gets numbered on
     screen; \`answer\` is always the last word, whatever the source. */
  // One test, asked once. Deciding this twice let the label say "ladder" while
  // the steps came from prose — which a mutation found and a reader would not.
  hasLadder(st){ return !!(st && Array.isArray(st.motes) && st.motes.length); },

  forStep(st){
    if(!st) return null;
    const steps=[];
    const fromLadder = this.hasLadder(st);

    if(fromLadder){
      // A ladder: each rung is a question with its own answer and reason.
      for(const m of st.motes){
        steps.push({ ask: String(m.q||'').replace(/<[^>]+>/g,' ').trim(),
                     got: m.a, why: m.ex || '' });
      }
    } else {
      for(const s of this.sentences(st.ex)) steps.push({ why:s });
    }

    if(!steps.length && st.ex) steps.push({ why:String(st.ex) });

    return {
      topic: st.topic || '',
      rule: (st.codex && st.codex.rule) || '',
      steps,
      answer: st.a,
      fig: st.figAnswer || null,
      // A ladder is authored; sentences are inferred. Worth saying which.
      source: fromLadder ? 'ladder' : 'explanation'
    };
  },

  // Every question can be worked. If this ever returns false the button must
  // not be offered, and the suite checks it never does across all seventy.
  has(st){ const w=this.forStep(st); return !!(w && w.steps.length && w.answer!=null); },

  render(w){
    if(!w) return '';
    return \`<div class="wkbox">
      \${w.rule?\`<div class="wkrule"><b>The rule</b><br>\${w.rule}</div>\`:''}
      <ol class="wksteps">
        \${w.steps.map(s=>\`<li>\${s.ask?\`<span class="wkask">\${s.ask}</span><br>\`:''}
          \${s.got?\`<span class="wkgot">\${s.got}</span>\${s.why?'<br>':''}\`:''}
          \${s.why?\`<span class="wkwhy">\${s.why}</span>\`:''}</li>\`).join('')}
      </ol>
      <div class="wkans">The answer is <b>\${w.answer}</b>.</div>
    </div>\`;
  }
};
...
const Rite = {
  ...
  GRADUATE: 3,
  /* Graduating off the ladder is not permanent. It is earned by clean ladders
     AND held by effective mastery, so a topic you graduated from and then left
     alone for a month arrives as a ladder again — the scaffolding comes back
     exactly when the memory of the procedure has gone. */
  graduated(key){
    const g=Game.s; if(!g) return false;
    const r=g.topicStats[key];
    return (g.riteWins && g.riteWins[key] || 0) >= this.GRADUATE
        && r && Mastery.eff(key) >= 0.7;
  },
  record(key, clean){
    const g=Game.s;
...
/* ------------------------------ skills & loadout -------------------------- */
/* The link that turns knowing the maths into something you press in a fight.

   A skill's abilities are NOT bought. You carry as many uses as your mastery
   of the skill's own branch of mathematics earns you, and they come back every
   fight — so the way to have more is to learn more, which is the whole claim
   the game is making. The bands are Dial Three's:

       weak  0 uses      shaky 1        steady 2        solid 3

   The one rule that keeps this from becoming a shortcut, straight out of the
   Faces and Loadouts note: no ability may reduce the number of questions in a
   fight. They make the same practice more survivable, or more learnable —
   never shorter. That is why none of these deals damage.

   Each ability draws on a STRAND rather than a single topic: a skill is a body
   of mathematics, not one question type. The strands here are the closest the
   present curriculum comes to each skill's real branch — Spellcraft is vector
   fields, Scrying is power laws, Smithing is allocation. This once said they
   would be re-pointed "when its setting is built"; every setting is built now,
   and re-pointing them was not the right call after all. A strand is what a
   skill is MADE of, and narrowing it to the topics of one place would tie an
   ability to a setting rather than to a body of mathematics — which is exactly
   what the ward-stone does deliberately, and only for the room. */
const SKILL_ABILITIES = [
  {id:'ward', nm:'Ward', ic:'🌀', skill:'Spellcraft', strand:'Vectors',
   ds:'Halves the next blow that lands on you.',
   ready(){ return !Battle.wardUp; },
   go(){ Battle.wardUp=true; UI.toast('🌀 A field folds around you.'); }},

  {id:'sight', nm:'Second Sight', ic:'👁️', skill:'Scrying', strand:'Limits & Derivatives',
   ds:'Read the rule for this topic without paying your combo.',
   ready(){ return Codex.has(Battle.cur) && !Battle.answered; },
   go(){ Codex.open(Battle.cur, ()=>{}); }},          // opened free: no combo cost

  {id:'steady', nm:'Steady Hand', ic:'🪨', skill:'Smithing', strand:'Applications',
   ds:'Your next wrong answer will not break your streak.',
   ready(){ return !Battle.steadyUp && Battle.combo>0; },
   go(){ Battle.steadyUp=true; UI.toast('🪨 Your grip settles.'); }},

  /* The first Foresight ability, and the only one so far used OUTSIDE a fight.
     It is spent at the fork, where the decision actually is — which is why its
     charges last a whole descent rather than refilling every room. Knowing what
     is below is the strongest thing a skill can give you, so it has to be
     scarce in the place it matters. */
  {id:'farsight', nm:'Farsight', ic:'🔭', skill:'Scrying', strand:'Multivariable & Series',
   where:'fork',
  ...
  /* The second Foresight ability, and deliberately not a better Farsight.
     Farsight is a point observation: one room, named exactly. Rumours is a
     summary of a sample — three rooms at once, counted and totalled, with no
     word about which is which. Knowing that the next stretch holds two fights
     and a champion tells you whether to commit to it; it does not tell you what
     is behind the next door. The two answer different questions, which is why
     carrying both is worth a slot each. */
  {id:'rumours', nm:'Rumours', ic:'🗣️', skill:'Rumours', strand:'Applications',
   where:'fork',
   ds:'At the fork, hear what the next three rooms hold — how many, and how bad, but not which.',
  ...
  /* The fourth Foresight ability, and the only one that reads a SHAPE rather
     than a place. Farsight names one room; Rumours totals three; Sighting gives
     two distances. Tracking gives none of those — it says how the danger out
     here is moving: what its period is, where in that period you are standing,
     and therefore whether the next stretch worsens or eases.

     That is worthless in a setting that only climbs, and it is the whole of the
     Wilds, where the herd circles. Four abilities, four different questions,
     and the reason each is worth a slot beside the others rather than instead
     of them. */
  {id:'tracking', nm:'Tracking', ic:'🐾', skill:'Tracking', strand:'Eigen & Subspaces',
   where:'fork',
   ds:'Read the swell: how long its turn is, where in it you stand, and whether what is ahead worsens or eases.',
   ready(){ return Dungeon.active && !Dungeon.tracked; },
   go(){ Dungeon.takeTracking(); }},

  /* The fifth and last Foresight ability, and the only one that reads
     something the player could not in principle have worked out. Farsight
     names a room already fixed by the seed; Rumours totals it; Sighting
     measures it; Tracking finds the period it repeats on. All four describe
     something derivable. The weather is DRAWN — a third of the Sea's leagues
     run rough on a roll — so a forecast is the one reading that sells
     information rather than arithmetic.

     It names which of the leagues ahead are rough, in order, because on the
     water the useful question is not how bad it gets but WHEN: sail the calm
     stretch and make shore before the blow. */
  {id:'weather', nm:'Weather-reading', ic:'🌦️', skill:'Weather-reading', strand:'Applications',
   where:'fork',
   ds:'Read the sky: which of the leagues ahead are running rough, and which are calm.',
   ready(){ return Dungeon.active && !Dungeon.forecastHere(); },
   go(){ Dungeon.readWeather(); }},
...
/* Scribing, at the bench. Pages in, recency out.
 *
 * The one rule, asserted here rather than assumed: this touches \`last\` and \`t\`
 * and NEVER \`m\` or \`seen\`. Recency is what decays effective mastery, so setting
 * it back restores what the knight had; the number underneath it is what they
 * actually know, and nothing bought with a currency may move that. */
const Scriptorium = {
  entry(id){ return SCRIPTORIUM.find(x=>x.id===id) || null; },

  /* The topics a binding would touch: the coldest first, which is the only
     order that makes the small bindings worth buying. "Coldest" is the gap
     between what a topic's mastery says and what it is currently worth after
     decay — a topic that has not faded has nothing to restore. */
  coldest(n){
    const g=Game.s; if(!g) return [];
    return Object.keys(g.topicStats)
      .filter(k => { const r=g.topicStats[k]; return r && r.seen>0; })
      .map(k => ({ k, lost:(g.topicStats[k].m||0) - Mastery.eff(k) }))
      .filter(x => x.lost > 0.001)
  ...
  bind(id){
    const e=this.entry(id), g=Game.s;
    if(!e) return;
    if((g.mats.page||0) < e.page){
      UI.toast(\`That needs \${e.page} pages — copy a passage in the Library.\`);
      return;
    }
    const keys=this.coldest(e.topics);
    if(!keys.length){ UI.toast('Nothing has gone cold enough to be worth binding.'); return; }
    g.mats.page -= e.page;
    const now=Date.now();
    for(const k of keys){
      const r=g.topicStats[k];
      // Recency only. \`m\` and \`seen\` are what the knight knows and are not for sale.
      r.last = g.qCount || 0;
      r.t = now;
    }
    Sfx.coin(); Haptic.win(); Game.save(); UI.renderShop();
    UI.toast(\`📄 \${keys.length} topic\${keys.length===1?'':'s'} bound — as fresh as the day you learned them.\`);
  }
};

/* Herbalism, at the bench. The mirror of the Forge: same shape, different

   currency, and one entry that is kept rather than spent. */
const Apothecary = {
  // What a draught is worth, as a multiplier. 1 until the satchel is brewed.
  draughtMul(){ return (Game.s && Game.s.brewed && Game.s.brewed.satchel) ? 1.5 : 1; },
  has(id){ const e=APOTHECARY.find(x=>x.id===id); return !!(e && e.owned && e.owned(Game.s)); },

  brew(id){
    const e=APOTHECARY.find(x=>x.id===id), g=Game.s;
    if(!e) return;
    if(e.owned && e.owned(g)) return;
    if((g.mats.herb||0) < e.herb){
      UI.toast(\`That needs \${e.herb} herbs — gather a thicket out in the Wilds.\`);
      return;
    }
    g.mats.herb -= e.herb;
    g.brewed = g.brewed || {};
    e.each(g);
    Sfx.coin(); Haptic.win(); Game.save(); UI.renderShop();
    if(e.keep) Celebrate.banner(e.nm.toUpperCase(), e.ds, '#7fc98a', ()=>{});
    else UI.toast(\`\${e.ic} \${e.nm} — made up and packed.\`);
  }
};

/* Alchemy's bench, and the last skill on the roll.
 *
 * Every other Material skill GATHERS something in one place and SPENDS it in
 * another. This one does neither: it CONVERTS, which is the third verb the
 * Material channel names and the only one nothing in the game did. It has no
 * room of its own for the same reason Smithing and Enchanting have none —
 * there is nothing to go and cut.
 *
 * The rate is the whole design, and it is diffusion rather than a price list.
 * Flux runs down a gradient, and the flow flattens the gradient that drives it:
 * pouring out of a deep pile into an empty one is efficient, pouring between
 * two level piles is barely worth doing, and every unit poured makes the next
 * one worth less. So the question is never "can I afford this" — you always
 * can — it is WHEN, and the answer depends on what you happen to be holding,
 * which is different every time.
 *
 * That is also what stops it dissolving the map. Settings exist to pull a
 * knight out of the place they like, and a free converter would remove that
 * pull entirely. This one prices it instead: the marginal rate can never reach
 * 1, so a round trip strictly loses and there is no cycle to farm — 100 pages
 * poured out and back comes home as 22, then 4, then nothing.
 *
 * The mathematics is the Sanctum's own. Gradients and partial derivatives are
 * literally what a diffusion is written in, and 'Multivariable & Series' is one
 * of the three strands that setting declares — which is the honest reason this
 * skill belongs there, rather than a chart having assigned it. */
const Crucible = {
  STRAND: 'Multivariable & Series',

  /* Floor and ceiling of the marginal rate, by mastery band. The floor matters
     more than it looks: a long pour collapses its own gradient, so most of what
     comes out at the end of one is paid at the floor, and scaling only the
     ceiling made mastery worth a rounding error. Neither ceiling reaches 1, and
     that is load-bearing rather than cautious — at 1 the bench mints material. */
  FLOORS: [0.12, 0.20, 0.30],
  CEILS:  [0.45, 0.65, 0.85],

  // The shares of a pile the bench offers to pour. A third, half, all of it.
  SHARES: [ {nm:'a third', frac:1/3}, {nm:'half', frac:0.5}, {nm:'the lot', frac:1} ],

  // What the bench is currently pointed at. Transient, and never saved.
  from: 'page',
  to: 'ore',
  ...
  reach(){
    const t = strandTopics(this.STRAND) || [];
    if(!t.length || !Game.s) return 0;
    let sum=0;
    for(const k of t) sum += Mastery.eff(k);
    return sum/t.length;
  },
  tier(){ const m=this.reach(); return m>=0.85 ? 2 : m>=0.6 ? 1 : 0; },

  /* The marginal rate at one instant: what the next single unit is worth.
     \`(s-d)/(s+d)\` is the gradient, normalised so it cannot run away when both
     piles are large, and clamped at zero so pouring uphill costs the floor
     rather than paying a negative. */
  rate(s, d){
    const t = this.tier(), f = this.FLOORS[t], c = this.CEILS[t];
    const tot = s + d;
    const grad = tot > 0 ? (s - d) / tot : 0;
    return f + (c - f) * Math.max(0, grad);
  },

  /* What pouring \`n\` units would yield, integrated one unit at a time because
     the rate moves while you pour. The target rises by what has already come
     out, so the gradient closes as it fills — which is the whole point, and is
     what makes the three buttons say genuinely different things rather than
     three multiples of one number. */
  quote(fromId, toId, n){
    let src = this.held(fromId);
    const dst0 = this.held(toId);
    n = Math.max(0, Math.min(Math.floor(n), src));
    let got = 0;
    for(let i=0; i<n; i++){
      got += this.rate(src, dst0 + got);
      src -= 1;
    }
    return { spend:n, get:Math.floor(got) };
  },
  ...
  // Bench plumbing: point it somewhere else and redraw. A pile cannot pour
  // into itself, so moving the source shoves the target aside if they collide.
  point(which, id){
    if(!this.mat(id)) return;
    if(which==='from'){
      this.from = id;
      if(this.to === id) this.to = (MATERIALS.find(m=>m.id!==id)||{}).id;
    } else {
...
/* A setting's own mathematics, flattened into a topic list. This is what makes
   the Sanctum feel like the Sanctum: its rooms prefer fields, flows and series,
   without ever excluding the rest — the scheduler treats it as a nudge, which
   is exactly the "relevance" signal it already weighs. */
/* One named strand, flattened the same way. \`settingTopics\` describes a PLACE —
   several strands, the character of the setting. This describes a SKILL: the
   single body of mathematics one skill is made of, for a room whose whole
   purpose is to practise it. */
function strandTopics(name){
  const s=STRANDS.find(x=>x[0]===name);
  return (s && s[1].length) ? s[1] : null;
}

function settingTopics(set){
  if(!set || !set.strands) return null;
  const out=[];
  for(const name of set.strands){
    const st=STRANDS.find(x=>x[0]===name);
    if(st) out.push(...st[1]);
  }
...
const Loadout = {
  ...
  topicsOf(a){ const s=STRANDS.find(x=>x[0]===a.strand); return s ? s[1] : []; },

  /* How well this knight knows the skill behind an ability: the mean of
     EFFECTIVE mastery across its strand, so a skill fades with the maths it is
     made of. Topics never met count as zero — a strand you have only dipped
     into does not read as mastered. */
  reach(a){
    const t=this.topicsOf(a);
    if(!t.length || !Game.s) return 0;
    let sum=0;
    for(const k of t) sum += Mastery.eff(k);
    return sum/t.length;
  },
  // Dial Three's bands, and the only place charge counts are decided.
  charges(a){
    const m=this.reach(a);
    return m>=0.85 ? 3 : m>=0.6 ? 2 : m>=0.35 ? 1 : 0;
  },
  band(a){ const m=this.reach(a); return Mastery.label(m); },

  chosen(){
    const g=Game.s;
    if(!g) return [];
    if(!Array.isArray(g.loadout)) g.loadout=[];
    return g.loadout.map(id=>this.byId(id)).filter(Boolean);
  },
  has(id){ return (Game.s.loadout||[]).indexOf(id)>=0; },
  // Tapping a chosen ability drops it; tapping another takes the oldest slot's
  ...
  toggle(id){
    ...
    if(i>=0) g.loadout.splice(i,1);
      ...
      g.loadout.push(id);
      while(g.loadout.length > this.SLOTS) g.loadout.shift();
    }
    Game.save();
    UI.renderGear();
  }
...
/* The wave engine: the pure "what does step N of an endless run look like"
   maths, lifted out of Arena so the escalation curve lives in one config object
   and a room's policy — the Arena is the first — only has to point at it. It
   owns nothing UI: rewards, healing, boons and screens stay with the room that
   consumes a wave. Kept deterministic under the seeded RNG, and its R.pick call
   order is load-bearing (it must match a foe's recorded appearance), so the
   picks below stay in name → art → colour order. */
const WaveEngine = {
  /* What an endless run draws from. Mastery.urgency deliberately over-weights
     unseen topics so a curated realm pool introduces its new material — but
     pointed at all seventy topics that same rule turns a run into things you
     have never been taught. So we ask what the player has met, and only fall
     back to everything if they have barely met anything. */
  pool(stats){
    const all = Object.keys(TOPIC_LABEL);
    const met = all.filter(k=>{ const r=stats[k]; return r && r.seen>0; });
    return met.length>=12 ? met : all;
  },

  /* Where in a curve's swell step \`n\` falls, as a multiplier. A curve without
     one returns 1 at every step, so this is a no-op everywhere but the Wilds.

     The swell is the Wilds' whole character: the herd circles, so danger rises
     and falls on a period instead of climbing forever. It multiplies health and
     reach and deliberately NOT reward — a trough that paid less would just be a
     slower stretch, where what it actually is is a safer one. */
  swell(n, c){
    if(!c.swell) return 1;
    return 1 + c.swell*Math.sin(2*Math.PI*(n-1)/c.swellEvery);
  },

  /* The foe for step \`n\` (1-based), shaped by escalation curve \`c\`.

     \`mul\` is an extra multiplier the CALLER knows about and the curve does
     not — the Sea's weather, which is rolled on a stream of its own rather
     than derived from n. It defaults to 1, so every other place is unaffected
     and the call sites that do not care need not pass it. */
  foe(n, c, mul){
    const champion = n % c.championEvery === 0;
    const sw = this.swell(n, c) * (mul || 1);
    const hp  = Math.round((c.hp0  + n*c.hpPer)  * sw * (champion?c.champHp:1));
    const atk = Math.round(c.atk0 * Math.pow(c.atkGrow, n-1) * sw * (champion?c.champAtk:1));
    return {
      nm:(champion?'':R.pick(c.adj)+' ')+R.pick(c.noun),
      art:R.pick(c.art), col:R.pick(c.col),
      hp, atk, boss:champion,
      gold:Math.round((c.gold0 + n*c.goldPer)*(champion?c.champReward:1)),
      xp:Math.round((c.xp0 + n*c.xpPer)*(champion?c.champReward:1))
    };
  }
};

/* The Arena's escalation curve. Every wave is a champion when it lands on a
   multiple of championEvery; champions hit the multipliers below. */
const ARENA_WAVES = {
  championEvery:5,
  hp0:200,  hpPer:95,  champHp:1.7,
  atk0:30,  atkGrow:1.085, champAtk:1.25,
  gold0:50, goldPer:18,
  xp0:30,   xpPer:10,
  champReward:2,
...
/* The Deep's own curve, and NOT the Arena's — which is the mistake this
   replaces. The Arena opens after the last realm boss, for a knight in the best
   gear in the game; the Deep opens after the FIRST, for one in a shortsword and
   a leather jerkin. Pointing the Deep at ARENA_WAVES meant a depth-one foe with
   295 health and 30 attack against a knight who swings for fourteen and has a
   hundred and forty-six health: unplayable, and nobody noticed because nothing
   measured it.

   These numbers come out of tools/balance.js, which plays thousands of runs at
   five accuracy levels and reads the break-even depth — the deepest point still
   worth pressing to — out of the results. The property they are chosen for is
   that break-even RISES with accuracy: a 40% player banks around depth three
   while a 95% player can justify pressing to twelve. That spread is what makes
   the press-on-or-leave fork an expression of skill rather than a formality,
   and \`npm run balance\` fails if a change to these flattens it. */
const DEEP_WAVES = {
  championEvery:5,
  hp0:40,  hpPer:18, champHp:1.7,
  atk0:8,  atkGrow:1.06, champAtk:1.25,
  gold0:50, goldPer:18,
  xp0:30,   xpPer:10,
  champReward:2,
  adj:ARENA_ADJ, noun:ARENA_NOUN, art:ARENA_ART, col:ARENA_COL
...
/* The cellar under the Keep — where a new knight is sent, and the only place
   in the game tuned so that losing is not really on the table. A fresh knight
   swings for nine, so these fall in three or four answers, and they hit for
   six against a hundred health: you would have to miss sixteen in a row. */
const CELLAR_WAVES = {
  championEvery:99,                        // nothing down here wears a crown
  hp0:22,  hpPer:12, champHp:1,
  atk0:6,  atkGrow:1.05, champAtk:1,
  gold0:20, goldPer:8,
  xp0:18,   xpPer:6,
  champReward:1,
  adj:ARENA_ADJ, noun:ARENA_NOUN, art:ARENA_ART, col:ARENA_COL
...
/* The Sanctum's curve. A shorter expedition than the Deep and a sharper one:
   its foes hit harder for their health, because the Sanctum is entered on
   purpose by a knight who already has a run behind them, not stumbled into. */
const SANCTUM_WAVES = {
  championEvery:4,
  hp0:55,  hpPer:24, champHp:1.6,
  atk0:11, atkGrow:1.07, champAtk:1.3,
  gold0:35, goldPer:14,
  xp0:40,   xpPer:14,
  champReward:2,
  adj:ARENA_ADJ, noun:ARENA_NOUN, art:ARENA_ART, col:ARENA_COL
...
/* The Wilds' curve, and the first that does not simply climb.
 *
 * Every other setting gets harder monotonically: the Deep and the Summit ramp,
 * the Summit additionally takes health away. The Wilds circles. A herd moves
 * through it on a period, so the ramp underneath is gentle and what sits on top
 * of it is a swell — nine rooms from crest to crest, a third again as dangerous
 * at the peak as at the trough.
 *
 * That is a different fork question, and it is the only reason this setting
 * exists. In the Deep you ask "can I take the next room". Here the next room is
 * not the point: what decides a run is whether the stretch AHEAD is rising or
 * easing, and one room's reading cannot tell you that. Which is what Tracking
 * is for — the ability whose whole job is to find a period and say where in it
 * you stand.
 *
 * It is a LONG place on purpose. tools/balance.js reads break-even out at
 * 10/12/18/20/27 as accuracy climbs — a wider spread than anywhere else, and a
 * walk of about three turns of the herd at the top end. That length is the
 * point rather than a side effect: a period nobody goes round twice is not a
 * period, and Tracking would have nothing to be right about in a setting you
 * leave before the ground has finished turning once.
 *
 * (Those numbers moved when the thickets landed. The Wilds shipped without
 * them, so a third of its rooms were fights that are now gathering — the
 * harness was measuring a harder place than the one being played, which is
 * exactly the drift the room cascade was made one function to prevent. The
 * figures above are the ones with thickets in.) */
const WILDS_WAVES = {
  championEvery:7,
  hp0:38,  hpPer:17, champHp:1.6,
  atk0:8,  atkGrow:1.07, champAtk:1.25,
  gold0:52, goldPer:19,
  xp0:30,   xpPer:11,
  champReward:2,
  swell:0.34, swellEvery:9,
  adj:ARENA_ADJ, noun:ARENA_NOUN, art:ARENA_ART, col:ARENA_COL
};

/* The Sea's curve, and the third kind of pressure.
 *
 * The Deep ramps. The Summit ramps and takes health away. The Wilds circles on
 * a period you can work out. The Sea does none of those: its foes are ordinary
 * and its danger is the WEATHER, which is a roll rather than a formula. Some
 * leagues run rough and hit half again as hard, and there is no arithmetic that
 * tells you which — only a forecast, which is what Weather-reading sells.
 *
 * That is deliberately the one place in the game where foresight is about
 * something the player could not in principle work out for themselves. Farsight
 * reads a room that is already fixed; Tracking reads a period that repeats. A
 * squall is drawn, and knowing which way it fell is worth paying for.
 *
 * The base curve is gentle for the same reason the Wilds' is: the weather is
 * already supplying the danger, and stacking two sources would leave no calm
 * stretch worth sailing into. tools/balance.js reads out the usual property. */
const SEA_WAVES = {
  championEvery:6,
  hp0:42,  hpPer:16, champHp:1.6,
  atk0:9,  atkGrow:1.062, champAtk:1.25,
  gold0:56, goldPer:20,
  xp0:32,   xpPer:12,
  champReward:2,
  squallChance:0.32, squallMul:1.5,
  adj:ARENA_ADJ, noun:ARENA_NOUN, art:ARENA_ART, col:ARENA_COL
};

/* The Library's curve, and the fourth kind of pressure.
 *
 * Its foes are the gentlest in the game and they barely grow, because they are
 * not what makes this place hard. What makes it hard is the STACKS: the deeper
 * in you read, the older the material, and every room is asked at a harder tier
 * than the one before until the whole shelf is at the top of it.
 *
 * That is the only pressure in the game that comes out of the mastery model
 * rather than out of a foe table, and it is the reason this is Loremaster's
 * home. Loremaster slows forgetting everywhere; here it slows the stacks
 * themselves, so a knight who has kept their reading warm gets further along
 * the shelf before the shelf gets hard. */
const LIBRARY_WAVES = {
  championEvery:8,
  hp0:34,  hpPer:11, champHp:1.5,
  atk0:7,  atkGrow:1.035, champAtk:1.2,
  gold0:48, goldPer:17,
  xp0:38,   xpPer:15,
  champReward:2,
  adj:ARENA_ADJ, noun:ARENA_NOUN, art:ARENA_ART, col:ARENA_COL
...
const VOICES = {
  ...
  sea:  { unit:'League', ic:'⛵', passage:'Open water, and no bottom under you.',
          on:'Sail on', out:'Make for shore', left:'Made shore', killer:'The sea',
          keeps:'The sea keeps you', ends:'voyage at league',
          reached:'Leagues sailed', again:'Set out again', risk:'until you make shore' },
  in:   { unit:'Shelf',  ic:'📚', passage:'The stacks go back further than the light does.',
          on:'Read on', out:'Take it to the desk', left:'Signed out', killer:'What you had forgotten',
          keeps:'The stacks keep you', ends:'reading at shelf',
          reached:'Shelves read', again:'Go back in', risk:'until you sign it out' }
};
...
/* A descent's setting: the curve it draws foes from, how its rooms are laid
   ...
   cannot end in a death — a first run has to finish, or it has taught nothing. */
const SETTINGS = {
  deep: {
    key:'deep', nm:'The Deep', waves:DEEP_WAVES,
    canDie:true, rooms:0, lockChance:.26, seamChance:.24, sigilChance:.16
  },
  cellar: {
    key:'cellar', nm:'the cellar', waves:CELLAR_WAVES,
    canDie:false, rooms:3, plan:['monster','lock','monster']
  },
  /* The Sanctum: six rooms, fixed, and every one of them drawn from the
     mathematics of fields and flows and things that grow without bound. It is
     where Scrying is practised, which is the only reason it pays essence
     rather than gold — you come here to get better at reading what is ahead,
     and the reward is the material that buys permanence. */
  sanctum: {
    key:'sanctum', nm:'the Sanctum', waves:SANCTUM_WAVES,
    canDie:true, rooms:6,
  ...
  summit: {
    ...
    /* The stone is commoner here than anywhere. The Summit kills by attrition
       rather than by any single foe, so a banked half-blow is worth more on the
       ridge than a chest is — and thin air means the knight who needs it most
       is the one who has climbed furthest to find it. */
  ...
  /* The Wilds: open country, walked rather than descended, and the first place
     whose danger does not only grow. A herd circles it on a period, so the
     ground ahead is sometimes rising and sometimes easing — and the fork stops
     being "can I take the next room" and becomes "is this stretch getting worse
     or better", which is a question no one-room reading can answer.

     Unlike the Tavern and the Summit it does pay a material. That was not true
     when it shipped — this setting arrived with Tracking and Beast taming, a
     Foresight and a Passage skill, and said in this comment that Herbalism
     wanted a gathering room of its own rather than something bolted on. It got
     one a slice later, and thickets are it. */
  wilds: {
    key:'wilds', nm:'the Wilds', waves:WILDS_WAVES,
    canDie:true, rooms:0, lockChance:.20, seamChance:.16, sigilChance:.14,
    /* Thickets grow here and nowhere else, which is what makes herbs the
  ...
  /* The Sea: sailed rather than descended, and the only place whose danger is
     drawn rather than derived. Roughly a third of its leagues run rough and hit
     half again as hard, on a roll nobody can work out from the shape of the
     curve — which is what makes a forecast worth buying, and what makes this
     the natural home for Weather-reading.

     Its material is Cargo, and it does have one: the hold, where unbanked gold
     is converted into freight that compounds every league and is lost whole in
     a wreck. That is the Sea's own arithmetic — insurance against interest —
     and it is why Dead reckoning, which salvages gold and never salvages
     cargo, belongs out here too. */
  sea: {
    key:'sea', nm:'the Sea', waves:SEA_WAVES,
    canDie:true, rooms:0, lockChance:.18, seamChance:.10, sigilChance:.12,
    holdChance:.26,
    voice:'sea',
    strands:['Integrals','Multivariable & Series','Applications'],
    foresight:'weather'
  },
  /* The Library: read into rather than descended, and the only place whose
     danger is what you have forgotten. Its foes are the gentlest in the game;
     what stops a knight is that every shelf is asked at a harder tier than the
     last until the whole stack is at the top of it.

     \`stackEvery\` is how many shelves it takes to lose a tier — divided by
     Loremaster's memory multiplier, so the skill that slows forgetting
     everywhere slows this too. That is what makes the Library its home rather
     than merely the place it was assigned to on a design chart.

     Its material is Pages, and what pages buy is the one thing no other
     currency touches: recency. Written down is remembered. */
  library: {
    key:'library', nm:'the Library', waves:LIBRARY_WAVES,
    canDie:true, rooms:0, lockChance:.16, seamChance:.08, sigilChance:.10,
    copyChance:.30,
    stackEvery:3,
    voice:'in',
    strands:['Limits & Derivatives','Matrices','Vectors'],
    /* The only setting whose answer is not a foresight at all. Every other
       place is survived by knowing what is ahead; this one is survived by not
       having forgotten what is behind, which is a Passage skill rather than a
       purchase. Naming a foresight here to satisfy the harness would have been
       a lie told to a test, so the harness learned a new word instead. */
    counter:'loremaster'
  }
 ...
 * are ordered once, rendered once, and a new setting is a line rather than
 * another forty lines of markup to keep in step. That paid off exactly as
 * hoped: the last three settings each landed as one line, in the right place,
...
const DESTINATIONS = [
  ...
  {id:'wilds', nm:'The Wilds', col:'#7fc98a', ic:'🌿',
   blurb:'Open country, and a herd that circles it. The danger out here rises and falls on a period rather than climbing — so what decides a walk is not the next room but whether the stretch ahead is worsening or easing.',
   open:()=>Dungeon.wildsOpen(),
   act:"Dungeon.descend('wilds')",
   label:()=>{ const f=(Game.s.bests&&Game.s.bests.wilds)||0;
               return 'Walk out into the Wilds'+(f?\` <span class="tag g">best: \${f} rooms</span>\`:''); },
   detail:()=>'Endless · danger comes in a swell · a fall loses the haul',
   sealed:'Clear the Eigen Citadel to find the way out'},

  {id:'sea', nm:'The Sea', col:'#5aa9e6', ic:'⛵',
   blurb:'Sailed, not descended, and the only place whose danger is <b>drawn</b> rather than derived. A third of its leagues run rough on a roll nobody can work out — which is what makes a forecast worth buying. Its hold turns gold into freight that compounds, and goes down whole in a wreck.',
   open:()=>Dungeon.seaOpen(),
   act:"Dungeon.descend('sea')",
   label:()=>{ const f=(Game.s.bests&&Game.s.bests.sea)||0;
               return 'Put out to sea'+(f?\` <span class="tag g">best: \${f} leagues</span>\`:''); },
   detail:()=>'Endless · squalls on a roll · 📦 freight compounds, and is not insured',
   sealed:'Clear the Spectral Reach to reach the coast'},

  {id:'library', nm:'The Library', col:'#c9b7ff', ic:'📚',
   blurb:'Read into rather than descended, and the only place whose danger is what you have <b>forgotten</b>. Its foes are the gentlest in the game; what stops you is that every shelf is asked older than the last. Loremaster slows the stacks themselves.',
   open:()=>Dungeon.libraryOpen(),
   act:"Dungeon.descend('library')",
   label:()=>{ const f=(Game.s.bests&&Game.s.bests.library)||0;
               return 'Go into the stacks'+(f?\` <span class="tag g">best: \${f} shelves</span>\`:''); },
   detail:()=>'Endless · the material ages as you read · 📄 pages buy back recency',
   sealed:'Clear the Infinite Expanse to be given a key'},
...
const Arena = {
  ...
  /* What an endless run draws from — the wave engine decides, from what this
     knight has met. */
  pool(){ return WaveEngine.pool(Game.s.topicStats); },
  ...
  foeFor(w){ return WaveEngine.foe(w, ARENA_WAVES); },
...
/* ------------------------------ room contract ----------------------------- */
/* A room kind is a table entry, the way generators and figure kinds already
   are. The shell (Dungeon, below) only ever calls these — it never looks
   inside a kind, so adding a kind is adding an entry. See the Room Contract
   design note. First Cave ships the monster kind here; the lock room lands in
   the next slice. build() is pure and seeded: the same (seed, depth) always
   builds the same room, which is what makes a run reproducible. */
const RoomKinds = {
  monster: {
    // Pure. Given the seeded stream and how deep we are, describe the room.
    build(rng, depth, set){
      const s = set || SETTINGS.deep;
      // The Sea's weather is the caller's business, not the curve's — see
      // Dungeon.rough. Everywhere else this multiplier is 1.
      return { kind:'monster', foe: WaveEngine.foe(depth, s.waves, Dungeon.rough(depth, s)),
               rough: Dungeon.isRough(depth, s) };
    },
    // Take the screen, play the fight out, then report an outcome exactly once.
    // The room decides nothing about the save or the map — it hands the shell
    // an outcome and the shell applies it.
    enter(spec, ctx, done){
      Battle.beginDungeon(spec.foe, won => done(won
        ? { status:'cleared', quality:1, topics:[],
            yield:{ gold:spec.foe.gold, xp:spec.foe.xp } }
  ...
  lock: {
    build(rng, depth, set){
      ...
      // Built under the room seed, so this pick must be pure — see Mastery.pick.
      const prefer = settingTopics(set) ||
                     (REALMS[Game.s.realm] ? REALMS[Game.s.realm].pool : null);
      const key  = Mastery.pick(pool, prefer, true);
      // The cellar's chest is a teaching chest: it never asks a hard one.
      // The Library's shelves are asked older the further in you read.
      const base = ((set && set.key==='cellar') ? 1 : clamp(1 + Math.floor(depth/3), 1, 3))
                 + Dungeon.stacks(depth, set);
  ...
  wager: {
    build(rng, depth, set){
      ...
      const prefer = settingTopics(set) ||
                     (REALMS[Game.s.realm] ? REALMS[Game.s.realm].pool : null);
      const key = Mastery.pick(pool, prefer, true);
      const base = clamp(1 + Math.floor(depth/3), 1, 3) + Dungeon.stacks(depth, set);
  ...
  /* The Tavern's other table, and the only room in the game that sells nothing
     but information. What it knows is what the remaining hands will ask about —
     which is worth exactly as much as it changes what you would have staked,
     and nothing at all if you were going to bet the same either way. */
  rumour: {
    build(rng, depth, set){ return { kind:'rumour', depth }; },
    enter(spec, ctx, done){ Rumour.begin(spec, ctx, done); },
  },

  /* A ward-stone — Spellcraft, and the room this skill has been owed since the
     first slice. Every other skill is practised somewhere: Delving has the
     seam, Scrying has the fork, Dice and Cards have the tables. Ward has only
     ever been a button, drawing on a strand nothing in the game ever asked
     about on purpose. This is that practice.

     Its questions come from the SKILL's strand rather than the setting's, which
     is what makes it practice rather than another room that happens to be here
     — and it reads that strand off the ability table, so if Spellcraft is ever
     re-pointed at its own topics the stone follows without being told.

     The shape of the decision is its own, too. The seam is all-or-nothing,
     declared before you start; the wager is a stake chosen before the riddle is
     seen. Inscribing is neither: each clean answer cuts a layer, and pressing on
     stakes the last layer you cut against one more. Even money at your own
     accuracy — the plainest bet in the game, in the one room that pays in
     survival rather than gold. */
  /* A thicket — Herbalism, and the Wilds' half of the material chain.
   *
   * Every other room asks you to earn something and then hands it over. This
   * one splits those into two decisions, because that is what its mathematics
   * is about: you cannot carry everything, so what matters is which of the
   * things in front of you are worth the room they take.
   *
   * First you earn BASKET SPACE, one square per clean answer, up to three, and
   * a miss ends the gathering rather than losing what you already earned — the
   * seam's bargain would be wrong here, since the room's decision is the second
   * half and a collapse would delete it. Then you spend that space: five plants
   * with different worths and different bulk, and a basket that will not hold
   * them all. The best haul is rarely the most valuable plant.
   */
  forage: {
    build(rng, depth, set){
      const strand = strandTopics(Forage.strand()) || [];
      const open = WaveEngine.pool(Game.s.topicStats);
      const pool = open.filter(k => strand.indexOf(k)>=0);
      const draw = pool.length ? pool : open;
      const base = clamp(1 + Math.floor(depth/4), 1, 3) + Dungeon.stacks(depth, set);
      const qs=[];
      for(let i=0;i<Forage.MAX_SPACE;i++){

        const key = Mastery.pick(draw, strand, true);
        qs.push(buildQuestion(key, Mastery.adjustDiff(key, base)));
      }
      /* The plants, drawn under the room seed so a resumed walk finds the same
    ...
    enter(spec, ctx, done){ Forage.begin(spec, ctx, done); },
  },

  /* A scriptorium — Scribing, and the only room where a mistake neither ends
   * the room nor costs a fixed thing.
   *
   * You copy a passage: several steps, and every step you get right adds to
   * what the copy is worth. A step you get wrong does not stop the copying and
   * does not take anything back — it costs PRECISION, and every step after it
   * is worth less because of it. Error accumulates rather than terminating,
   * which is the one shape none of the other rooms has: the seam collapses, the
   * ward-stone cracks a layer, the thicket stops. Here you simply end up with a
   * poorer copy, and the further in the mistake was, the more of the work after
   * it was spent at reduced worth.
   */
  copy: {
    build(rng, depth, set){
      const strand = strandTopics(Scribe.strand()) || [];
      const open = WaveEngine.pool(Game.s.topicStats);
      const pool = open.filter(k => strand.indexOf(k)>=0);
      const draw = pool.length ? pool : open;
      const base = clamp(1 + Math.floor(depth/4), 1, 3) + Dungeon.stacks(depth, set);
      ...
      return { kind:'copy', qs, depth };
    },
    enter(spec, ctx, done){ Scribe.begin(spec, ctx, done); },
  },

  /* A cargo hold — Cargo, and the Sea's own arithmetic.

   *
   * Every other material in the game is a thing you gather. This one is a thing
   * you CONVERT: unbanked gold, turned into freight that compounds every league
   * you go on sailing. The catch is what a wreck does. Gold in the pot is partly
   * salvaged by Dead reckoning; cargo is never salvaged, and goes down whole.
   *
   * So the room is one number against another — interest against insurance —
   * and both are shown, because a decision you cannot price is not one. It is
   * the only room whose worth depends on how much further you mean to sail,
   * which is why it belongs in an endless setting rather than a fixed one. */
  hold: {
    build(rng, depth, set){
      return { kind:'hold', depth };
    },
    enter(spec, ctx, done){ Hold.begin(spec, ctx, done); },
  },

  sigil: {
    build(rng, depth, set){
      /* Every other room passes its topics as \`prefer\`, which the scheduler
         treats as a NUDGE — it weights the draw and nothing more, so a Sanctum
         room asking about matrices now and then is the system working. This
         room filters instead, and that difference is the whole point of it: a
         place that practises Spellcraft two riddles in three is not where
         Spellcraft is practised. Inside the strand the scheduler still chooses
         normally, so the stone asks about the vectors you are worst at.

         The filter falls back to the open pool if it would empty it, so a
         knight who has somehow met none of the strand still gets a room rather
         than a crash. */
      const open = WaveEngine.pool(Game.s.topicStats);
      const strand = strandTopics(Sigil.strand()) || [];
      const pool = open.filter(k => strand.indexOf(k)>=0);
      const draw = pool.length ? pool : open;
      const base = clamp(1 + Math.floor(depth/4), 1, 3) + Dungeon.stacks(depth, set);
      const qs=[];
      ...
      return { kind:'sigil', qs, depth };
    },
    enter(spec, ctx, done){ Sigil.begin(spec, ctx, done); },
  },
};
...
/* --------------------------- the scriptorium ------------------------------ */
/* Scribing's UI half, and the only room in the game where being wrong is
   neither fatal nor free. Each clean step adds a page; each miss takes a slice
   off the PRECISION every later step is written at. So a slip on the first line
   costs more than the same slip on the last, which is what finite precision
   actually means and what nothing else in the game says. */
const Scribe = {
  STEPS: 4,
  // What each clean step is worth, before precision is applied.
  PER_STEP: 4,
  // What a slip costs, as a share of the precision that remains.
  SLIP: 0.3,

  spec:null, ctx:null, done:null, resolved:false, at:0, pages:0, precision:1, slips:0,
  token:0,

  /* Every step of a copy is reached through a timer, and this module is a
     singleton. \`resolved\` stops a finished passage paying twice, but it does
     not stop a timer left over from ONE passage landing inside the next — which
     would resolve a room the player is halfway through. So each begin() takes a
     token, and a deferred step that finds the token moved simply does nothing.
     Cheaper and surer than tracking handles to clear. */
  later(fn, ms){
    const tok=this.token;
    setTimeout(()=>{ if(this.token===tok) fn(); }, ms);
  },


  /* The strand this room drills, read off the setting rather than written down
     twice — the same rule the thicket follows. Scribing has no pressable
     ability to hang it on, so the Library's own first strand is the source. */
  strand(){ return (SETTINGS.library.strands && SETTINGS.library.strands[0]) || 'Limits & Derivatives'; },

  begin(spec, ctx, done){
    this.spec=spec; this.ctx=ctx; this.done=done;
    this.resolved=false; this.at=0; this.pages=0; this.precision=1; this.slips=0;
    this.token++;                       // anything still in flight is now stale
    UI.go('s-lock');
    this.open();
  ...
  open(){
    document.getElementById('lockBody').innerHTML=\`
      ...
      <div class="panel">
        <div class="row" style="justify-content:space-between;display:flex">
          <span class="pill">📄 pages ×\${Game.s.mats.page}</span>
          <span class="pill">✒️ \${this.STEPS} lines</span>
        </div>
        <h2 style="font-size:18px;margin-top:8px">A scriptorium</h2>
        <div class="sub">Copy the passage, a line at a time. Every line you get right is
          worth <b>\${this.PER_STEP}</b> pages.<br><br>
          A line you get wrong does not stop you and takes nothing back — it costs
          <b>\${Math.round(this.SLIP*100)}%</b> of your precision, and every line after it is
          worth that much less. A slip on the first line is dearer than the same slip on the
          last, which is the whole of the arithmetic here.</div>
      </div>
      <button class="btn gold" onclick="Scribe.ask()">✒️ Take up the pen</button>
      <div class="center" style="margin-top:6px">
        <span class="kbd" onclick="Scribe.walkAway()">Leave the passage uncopied</span>
      </div>
      <div style="height:16px"></div>\`;
  },

  /* Every step of the copying is reached through a timer, and a room that has
     already reported must not still be rendering or still be paying. Without
     this a late timer from a finished passage can add pages to the next one —
     which is exactly what a resolved room reporting twice would look like. */
  ask(){
    if(this.resolved) return;
    const q=this.spec.qs[this.at];
    document.getElementById('lockBody').innerHTML=\`
      ...
      <div class="panel">
        <div class="row" style="justify-content:center;gap:6px;display:flex;margin-bottom:8px">
          \${this.spec.qs.map((x,i)=>
            \`<span class="rp \${i<this.at?'done':i===this.at?'now':''}"></span>\`).join('')}
        </div>
        <h2 style="font-size:17px">\${q.topic} <span class="tag">line \${this.at+1}/\${this.STEPS}</span></h2>
        <div class="sub">📄 <b>\${Math.floor(this.pages)}</b> pages so far, at
          <b>\${Math.round(this.precision*100)}%</b> precision — this line is worth
          <b>\${Math.floor(this.PER_STEP*this.precision)}</b>.</div>
        <hr>
        <div style="font-size:17px;text-align:center;line-height:1.5">\${q.q}</div>
        <div id="scribeFig"></div>
        <div id="scribeChoices" style="margin-top:10px"></div>
      </div>
      <div id="lockOut"></div>
      <div style="height:16px"></div>\`;
    Figure.render(q.fig, document.getElementById('scribeFig'));
  ...
  answer(btn, choice){
    if(this.resolved) return;
    const q=this.spec.qs[this.at], ok=(choice===q.a);
    document.querySelectorAll('#scribeChoices .choice').forEach(b=>{
      b.classList.add('faded');
      if(b.dataset.correct==='1') b.classList.add('right');
    });
    if(!ok){ btn.classList.remove('faded'); btn.classList.add('wrong'); }
    ...
    if(ok) this.pages += this.PER_STEP * this.precision;
    else { this.precision *= (1 - this.SLIP); this.slips++; }
    this.at++;

    const slot=document.getElementById('lockOut') || document.getElementById('lockBody');
    if(!ok) slot.innerHTML=
      \`<div class="panel center">
         <div class="sub">\${q.why[choice]||'that is not what the passage says'}</div>
         <div class="small" style="margin-top:6px">The answer was <b>\${q.a}</b>. \${q.ex||''}</div>
         <div class="small" style="margin-top:6px">Precision falls to
           <b>\${Math.round(this.precision*100)}%</b>.</div>
       </div>\`;

    if(this.at >= this.STEPS){ this.later(()=>this.finishCopy(), ok?700:1500); return; }
    this.later(()=>this.ask(), ok?700:1500);

  },

  finishCopy(){
    if(this.resolved) return;
    const pages=Math.floor(this.pages);

    Game.s.mats.page += pages;
    Game.save();
    const clean=this.slips===0;
    this.finish({ status:'cleared', quality:pages?1:0,
                  topics:this.spec.qs.map(x=>x.key), yield:{},
                  copy:{pages, slips:this.slips} },
      \`<div class="center crest">\${clean?'📜✨':'📜'}</div>
       <div class="panel center">
         <h2 style="color:\${clean?'var(--green)':'var(--text)'}">
           \${clean?'A clean copy':'The copy is finished'}</h2>
         <div class="sub">\${clean
           ? 'Not a line out of place, and every one of them at full worth.'
           : \`\${this.slips} slip\${this.slips===1?'':'s'}, and everything after them written at
              \${Math.round(this.precision*100)}%.\`}</div>
         \${pages?\`<hr><div style="font-weight:800;line-height:1.9">📄 +\${pages} pages</div>
           <div class="small" style="margin-top:6px">Pages are bound at the Keep — into the one
             thing gold cannot buy back, which is what you used to know.</div>\`:''}
       </div>\`);
  },

  walkAway(){
    this.finish({ status:'cleared', quality:0, topics:[], yield:{}, copy:{pages:0, slips:0} },
      \`<div class="center crest">✒️</div>
       <div class="panel center"><h2>You leave the passage uncopied</h2>
       <div class="sub">It has waited this long.</div></div>\`);
  },

  finish(outcome, html){
    this.resolved=true;
    const out=document.getElementById('lockOut') || document.getElementById('lockBody');
    out.innerHTML=html+\`<button class="btn gold" id="scribeGo" style="margin-top:12px">Onward →</button>\`;
    document.getElementById('scribeGo').onclick=()=>{
      const d=this.done; this.done=null; if(d) d(outcome);
    };
    if(outcome.copy && outcome.copy.pages>0){ Sfx.coin(); Haptic.win(); }
  }
};

/* ------------------------------- the hold --------------------------------- */

/* Cargo's UI half. Owns nothing about the run: it reports how much gold became
   freight, and the shell moves it. */
const Hold = {
  // What a league of sailing adds to freight already stowed.
  GROWTH: 0.12,
  // The shares of the pot on offer. A third, a half, all of it.
  SHARES: [
    {nm:'a third', frac:1/3},
    {nm:'half',    frac:0.5},
    {nm:'the lot', frac:1}
  ...
  /* What a stowed coin is worth after \`n\` more leagues, and what the same coin
     is worth left in the pot — which is not its face value, because a wreck
     takes all of it but what Dead reckoning saves. Both numbers are quoted at
     the same n, so the comparison on the buttons is a fair one. */
  grown(gold, n){ return Math.round(gold * Math.pow(1+this.GROWTH, Math.max(0, n))); },
  breakEven(){
    /* Freight beats gold when compounding outruns the salvage you give up.
       Cargo is lost whole; gold keeps \`salvage\` of itself through a wreck. So
       stowing wins after n leagues when (1+g)^n > 1 ... and the honest number to
       show is how many leagues of sailing it takes for the growth to cover what
       salvage would have returned on a wreck. */
    const save = Passage.salvage();
    if(save<=0) return 0;                       // nothing to give up: stow at once
    return Math.ceil(Math.log(1/(1-save)) / Math.log(1+this.GROWTH));
  },
  ...
  offer(){
    const pot=Math.floor(Dungeon.run.unbanked.gold);
    const stowed=Math.floor(Dungeon.run.cargo||0);
    const save=Math.round(Passage.salvage()*100);
    const be=this.breakEven();
    const opt=(sh)=>{
      const take=Math.floor(pot*sh.frac);
      if(take<1) return '';
      return \`<button class="btn \${sh.frac===1?'gold':''}" onclick="Hold.stow(\${sh.frac})">
        ⚓ Stow \${sh.nm} — \${take} gold
        <span style="font-size:12px;font-weight:700;margin-left:6px;
                     color:\${sh.frac===1?'rgba(40,26,4,.72)':'var(--dim)'}"
        >worth \${this.grown(take,5)} in five leagues</span>
      </button>\`;
    };
    document.getElementById('lockBody').innerHTML=\`
      <div class="center crest">⚓</div>
      <div class="panel">
        <div class="row" style="justify-content:space-between;display:flex">
          <span class="pill">🎒 \${pot} gold in the pot</span>
          <span class="pill">📦 \${stowed} stowed</span>
        </div>
        <h2 style="font-size:18px;margin-top:8px">A cargo hold</h2>
        <div class="sub">Gold turned into freight grows <b>\${Math.round(this.GROWTH*100)}%</b>
          a league, every league you keep sailing.<br><br>
          It is not insured. A wreck takes cargo whole, where
          \${save ? \`Dead reckoning would carry <b>\${save}%</b> of the pot out with you\`
                 : 'the pot is lost either way — you have no Dead reckoning to lose'}.
          \${be ? \`On those terms the freight is ahead after about <b>\${be}</b> more league\${be===1?'':'s'}.\`
               : 'With nothing to salvage, there is nothing to weigh: stow it.'}</div>
      </div>
      \${this.SHARES.map(sh=>opt(sh)).join('')}
      <div class="center" style="margin-top:6px">
        <span class="kbd" onclick="Hold.walkAway()">Keep it all in the pot</span>
      </div>
      <div style="height:16px"></div>\`;
  },
  ...
  stow(frac){
    if(this.resolved) return;
    const take=Math.floor(Math.floor(Dungeon.run.unbanked.gold)*frac);
    if(take<1){ this.walkAway(); return; }
    this.finish({ status:'cleared', quality:1, topics:[], yield:{},
                  hold:{stowed:take} },
      \`<div class="center crest">📦</div>
       <div class="panel center">
         <h2 style="color:var(--green)">Stowed and lashed down</h2>
         <div class="sub">\${take} gold is freight now. It grows
           \${Math.round(this.GROWTH*100)}% a league — and goes down with the ship.</div>
       </div>\`);
  },

  walkAway(){
    this.finish({ status:'cleared', quality:0, topics:[], yield:{}, hold:{stowed:0} },
      \`<div class="center crest">⚓</div>
       <div class="panel center"><h2>The hold stays empty</h2>
       <div class="sub">Every coin stays in the pot, where a wreck can only take
         what Dead reckoning cannot hold onto.</div></div>\`);
  },

  finish(outcome, html){
    this.resolved=true;
    const out=document.getElementById('lockOut') || document.getElementById('lockBody');
    out.innerHTML=html+\`<button class="btn gold" id="holdGo" style="margin-top:12px">Onward →</button>\`;
    document.getElementById('holdGo').onclick=()=>{
      const d=this.done; this.done=null; if(d) d(outcome);
    };
    if(outcome.hold && outcome.hold.stowed>0){ Sfx.coin(); Haptic.win(); }
  }
};

/* ------------------------------ the thicket ------------------------------- */
/* Herbalism's UI half, and the only room in the game with two decisions in it.
 *
 * The riddles buy CAPACITY rather than loot, and the loot is then chosen. That
 * split is the point: knowing the mathematics gets you a bigger basket, and
 * what you do with the basket is a separate question the mathematics does not
 * answer for you. A knight who answers everything correctly and then fills the
 * basket badly walks out with less than one who answered less and chose well.
 */
const Forage = {
  MAX_SPACE:3, PLANTS:5,
  NAMES:['eigenroot','vetch','kernel-moss','spectral fern','null-nettle',
         'basis-thistle','trace-lichen','orthogonal ivy'],
  ICONS:['🌿','🍃','🌱','☘️','🍀'],

  spec:null, ctx:null, done:null, resolved:false, at:0, space:0, taken:null,

  /* The strand this room drills, read off the skill it belongs to rather than
     written down twice — the same rule the ward-stone follows. Herbalism has no
     pressable ability to hang it on, so the setting's own first strand is the
     honest source: the Wilds is where Herbalism is practised. */
  strand(){ return (SETTINGS.wilds.strands && SETTINGS.wilds.strands[0]) || 'Eigen & Subspaces'; },

  begin(spec, ctx, done){
    this.spec=spec; this.ctx=ctx; this.done=done;
    this.resolved=false; this.at=0; this.space=0; this.taken=[];
    UI.go('s-lock');
    this.open();
  },
  ...
  open(){
    document.getElementById('lockBody').innerHTML=\`
      ...
      <div class="panel">
        <div class="row" style="justify-content:space-between;display:flex">
          <span class="pill">🌿 herbs ×\${Game.s.mats.herb}</span>
          <span class="pill">🧺 basket 0/\${this.MAX_SPACE}</span>
        </div>
        <h2 style="font-size:18px;margin-top:8px">A thicket</h2>
        <div class="sub">More grows here than you can carry. Each riddle you answer clears
          another square of basket — a wrong one ends the gathering, but you keep the room
          you have already made.<br><br>
          Then you choose. Every plant takes its own space, and the dearest is not always
          worth the room it wants.</div>
      </div>
      <button class="btn gold" onclick="Forage.ask()">🌿 Start gathering</button>
      <div class="center" style="margin-top:6px">
        <span class="kbd" onclick="Forage.walkAway()">Walk past the thicket</span>
      </div>
      <div style="height:16px"></div>\`;
  },
  ...
  ask(){
    ...
    document.getElementById('lockBody').innerHTML=\`
      ...
      <div class="panel">
        <div class="row" style="justify-content:center;gap:6px;display:flex;margin-bottom:8px">
          \${this.spec.qs.map((x,i)=>
            \`<span class="rp \${i<this.space?'done':i===this.at?'now':''}"></span>\`).join('')}
        </div>
        <h2 style="font-size:17px">\${q.topic} <span class="tag">basket \${this.space}/\${this.MAX_SPACE}</span></h2>
        <div class="sub">Answer this and the basket takes one more.</div>
        <hr>
        <div style="font-size:17px;text-align:center;line-height:1.5">\${q.q}</div>
        <div id="forageFig"></div>
        <div id="forageChoices" style="margin-top:10px"></div>
      </div>
      <div id="lockOut"></div>
      <div style="height:16px"></div>\`;
    Figure.render(q.fig, document.getElementById('forageFig'));
  ...
  answer(btn, choice){
    if(this.resolved) return;
    const q=this.spec.qs[this.at], ok=(choice===q.a);
    document.querySelectorAll('#forageChoices .choice').forEach(b=>{
      b.classList.add('faded');
      if(b.dataset.correct==='1') b.classList.add('right');
    });
    if(!ok){ btn.classList.remove('faded'); btn.classList.add('wrong'); }
    ...
    if(!ok){
      /* A miss ends the gathering and takes nothing back. The seam's all-or-
         nothing would be wrong here: the room's real decision is the choosing,
         and a collapse would delete it rather than sharpen it. */
      // Same defensive shape finish() uses: the explanation goes under the
      // question when there is a slot for it, and into the body when there is
      // not, rather than throwing on a null.
      const slot=document.getElementById('lockOut') || document.getElementById('lockBody');
      slot.innerHTML=
        \`<div class="panel center">
           <div class="sub">\${q.why[choice]||'that is not what grows here'}</div>
           <div class="small" style="margin-top:6px">The answer was <b>\${q.a}</b>. \${q.ex||''}</div>
         </div>\`;
      setTimeout(()=>this.choose(), 1400);
      return;
    }
    this.space++; this.at++;
    if(this.at >= this.MAX_SPACE){ setTimeout(()=>this.choose(), 700); return; }
    setTimeout(()=>this.ask(), 700);
  },

  // What is in the basket, and what is left of it.
  held(){ return this.taken.reduce((n,i)=>n + this.spec.plants[i].bulk, 0); },
  worth(){ return this.taken.reduce((n,i)=>n + this.spec.plants[i].worth, 0); },
  room(){ return this.space - this.held(); },

  toggle(i){
    if(this.resolved) return;
    const at=this.taken.indexOf(i);
    if(at>=0) this.taken.splice(at,1);
    else {
      if(this.spec.plants[i].bulk > this.room()) return;   // it will not fit
      this.taken.push(i);
    }
    this.choose();
  },
  ...
  choose(){
    ...
    if(this.space<1){
      this.finish({ status:'cleared', quality:0, topics:[], yield:{},
                    forage:{herbs:0, space:0, took:0} },
        \`<div class="center crest">🌿</div>
         <div class="panel center"><h2>Nothing to carry it in</h2>
         <div class="sub">The basket never opened. The thicket keeps what it has.</div></div>\`);
      return;
    }
    const rows=this.spec.plants.map((pl,i)=>{
      const on=this.taken.indexOf(i)>=0;
      const fits = on || pl.bulk <= this.room();
      return \`<div class="item \${on?'equipped':''}" style="\${fits?'':'opacity:.45'}"
                   onclick="Forage.toggle(\${i})">
        <div class="ic">\${pl.ic}</div>
        <div style="flex:1"><div class="nm">\${pl.nm}</div>
          <div class="ds">🌿 \${pl.worth} herbs · takes \${pl.bulk} square\${pl.bulk===1?'':'s'}</div></div>
        \${on?'<span class="pill">packed</span>':fits?'':'<span class="pill">no room</span>'}
      </div>\`;
    }).join('');
    document.getElementById('lockBody').innerHTML=\`
      <div class="center crest">🧺</div>
      <div class="panel">
        <div class="row" style="justify-content:space-between;display:flex">
          <span class="pill">🧺 \${this.held()}/\${this.space} full</span>
          <span class="pill">🌿 \${this.worth()} herbs packed</span>
        </div>
        <h2 style="font-size:18px;margin-top:8px">What will you carry?</h2>
        <div class="sub">\${this.space} square\${this.space===1?'':'s'} of basket, and more here
          than fits in it. Tap to pack and unpack.</div>
      </div>
      \${rows}
      <button class="btn gold" onclick="Forage.leave()">🧺 Carry it out — \${this.worth()} herbs</button>
      <div id="lockOut"></div>
      <div style="height:16px"></div>\`;
  },

  leave(){
    const herbs=this.worth(), took=this.taken.length;
    Game.s.mats.herb += herbs;
    Game.save();
    this.finish({ status:'cleared', quality:herbs?1:0,
                  topics:this.spec.qs.slice(0, this.at).map(x=>x.key), yield:{},
                  forage:{herbs, space:this.space, took} },
      \`<div class="center crest">🌿✨</div>
       <div class="panel center">
         <h2 style="color:var(--green)">\${took?'You pack up and move on':'You leave it standing'}</h2>
         <div class="sub">\${took} plant\${took===1?'':'s'} in \${this.held()} of \${this.space} squares.</div>
         \${herbs?\`<hr><div style="font-weight:800;line-height:1.9">🌿 +\${herbs} herbs</div>
           <div class="small" style="margin-top:6px">Herbs are made up at the Keep — into
             draughts, and into one thing gold cannot buy.</div>\`:''}
       </div>\`);
  },
  ...
  walkAway(){
    this.finish({ status:'cleared', quality:0, topics:[], yield:{},
                  forage:{herbs:0, space:0, took:0} },
      \`<div class="center crest">🌿</div>
       <div class="panel center"><h2>You walk past the thicket</h2>
       <div class="sub">It will still be growing when somebody else comes by.</div></div>\`);
  },

  finish(outcome, html){
    this.resolved=true;
    const out=document.getElementById('lockOut') || document.getElementById('lockBody');
    out.innerHTML=html+\`<button class="btn gold" id="forageGo" style="margin-top:12px">Onward →</button>\`;
    document.getElementById('forageGo').onclick=()=>{
      const d=this.done; this.done=null; if(d) d(outcome);
    };
    if(outcome.forage && outcome.forage.herbs>0){ Sfx.coin(); Haptic.win(); }
  }
};

/* ---------------------------- the ward-stone ------------------------------ */
/* Spellcraft's UI half, and the first room in the game that pays in survival.
 *
 * A layer is a banked Ward: it halves one blow, it spends itself, and it lasts
 * the descent rather than the fight. That last part is what makes it worth
 * carrying past the room — and what puts it at risk, because a ward rides on
 * the run exactly as unbanked gold does. Fall and it is gone with everything
 * else, which is the Deep's whole bargain applied to something other than coin.
 *
 * It obeys the rule every skill obeys: pressing on adds a question, so no fight
 * is ever one riddle shorter for having stood here.
 */
const Sigil = {
  MAX:3,
  spec:null, ctx:null, done:null, resolved:false, at:0, layers:0,

  /* Which mathematics a ward is made of — read off the ability rather than
     written down twice. Ward is the skill's only pressable, so its strand IS
     Spellcraft's strand, and re-pointing one re-points the other. */
  strand(){ const a=Loadout.byId('ward'); return (a && a.strand) || 'Vectors'; },

  begin(spec, ctx, done){
    this.spec=spec; this.ctx=ctx; this.done=done;
    this.resolved=false; this.at=0; this.layers=0;
    UI.go('s-lock');                       // the same plain non-combat screen
    this.open();
  },

  // How many wards are already riding on this descent, before this stone.
  carried(){ return Dungeon.wards(); },
  ...
  open(){
    document.getElementById('lockBody').innerHTML=\`
      ...
      <div class="panel">
        <div class="row" style="justify-content:space-between;display:flex">
          <span class="pill">🪬 \${this.strand()}</span>
          <span class="pill">🌀 wards ×\${this.carried()}</span>
        </div>
        <h2 style="font-size:18px;margin-top:8px">A ward-stone</h2>
        <div class="sub">Cut a ward into the stone and carry it down with you: each layer
          halves one blow, whenever it lands, for the rest of the descent.<br><br>
          Every riddle you answer cuts a layer. Press on for another and you stake the
          last one you cut — miss, and it cracks along with the one you were reaching for.</div>
      </div>
      <button class="btn gold" onclick="Sigil.ask()">🌀 Begin the inscription</button>
      <div class="center" style="margin-top:6px">
        <span class="kbd" onclick="Sigil.walkAway()">Leave the stone unmarked</span>
      </div>
      <div style="height:16px"></div>\`;
  },
  ...
  ask(){
    ...
    document.getElementById('lockBody').innerHTML=\`
      ...
      <div class="panel">
        <div class="row" style="justify-content:center;gap:6px;display:flex;margin-bottom:8px">
          \${this.spec.qs.map((x,i)=>
            \`<span class="rp \${i<this.layers?'done':i===this.at?'now':''}"></span>\`).join('')}
        </div>
        <h2 style="font-size:17px">\${q.topic} <span class="tag">layer \${this.at+1}/\${this.MAX}</span></h2>
        <div class="sub">\${this.layers
          ? \`<b>\${this.layers}</b> layer\${this.layers===1?'':'s'} cut. This one is staked against it.\`
          : 'Nothing cut yet. This one costs you nothing to try.'}</div>
        <hr>
        <div style="font-size:17px;text-align:center;line-height:1.5">\${q.q}</div>
        <div id="sigilFig"></div>
        <div id="sigilChoices" style="margin-top:10px"></div>
      </div>
      <div id="lockOut"></div>
      <div style="height:16px"></div>\`;
    Figure.render(q.fig, document.getElementById('sigilFig'));
  ...
  answer(btn, choice){
    if(this.resolved) return;
    const q=this.spec.qs[this.at], ok=(choice===q.a);
    document.querySelectorAll('#sigilChoices .choice').forEach(b=>{
      b.classList.add('faded');
      if(b.dataset.correct==='1') b.classList.add('right');
    });
    if(!ok){ btn.classList.remove('faded'); btn.classList.add('wrong'); }
    Game.recordAnswer(q.key, ok, 1);       // the stone teaches either way
    Game.save();

    if(!ok){
      /* The stake, paid. One layer cracks — never below nothing, so the first
         riddle is genuinely free and a player meeting the room for the first
         time cannot be punished for trying it. */
      const lost=this.layers>0 ? 1 : 0;
      this.layers-=lost;
      this.seal(
        \`<div class="center crest">🌀💢</div>
         <div class="panel center">
           <h2 style="color:var(--red)">The line breaks</h2>
           <div class="sub">\${q.why[choice]||'that was not the shape of the field'}</div>
           <hr>
           <div class="small">The answer was <b>\${q.a}</b>. \${q.ex||''}</div>
           <div class="small" style="margin-top:8px">\${lost
             ? 'A cut layer cracks with it.' : 'Nothing was cut yet, so nothing is lost.'}</div>
         </div>\`);
      return;
    }
    ...
    if(this.at >= this.MAX){
      this.seal(
        \`<div class="center crest">🌀✨</div>
         <div class="panel center">
           <h2 style="color:var(--green)">The stone is full</h2>
           <div class="sub">Three layers, and no room on it for a fourth.</div>
         </div>\`);
      return;
    }
    setTimeout(()=>this.offer(), 650);
  },

  /* The fork in miniature, and the reason this room exists: at every step the
     player is choosing between a ward they hold and a ward they might hold, at
     odds they set themselves by knowing the mathematics. Both numbers are on
     the buttons, because a decision you cannot price is not one. */
  offer(){
    if(this.resolved) return;
    document.getElementById('lockBody').innerHTML=\`
      <div class="center crest">🌀</div>
      <div class="panel center">
        <div class="row" style="justify-content:center;gap:6px;display:flex;margin-bottom:8px">
          \${this.spec.qs.map((x,i)=>
            \`<span class="rp \${i<this.layers?'done':''}"></span>\`).join('')}
        </div>
        <h2 style="font-size:18px;color:var(--green)">The line holds</h2>
        <div class="sub"><b>\${this.layers}</b> layer\${this.layers===1?'':'s'} cut.
          One more riddle would make it <b>\${this.layers+1}</b> — or crack it back to
          <b>\${this.layers-1}</b>.</div>
      </div>
      <button class="btn gold" onclick="Sigil.ask()">🌀 Cut deeper — stake \${this.layers} for \${this.layers+1}</button>
      <button class="btn ghost" onclick="Sigil.sealNow()">🪬 Seal it at \${this.layers}</button>
      <div style="height:16px"></div>\`;
  },
  ...
  sealNow(){
    this.seal(
      \`<div class="center crest">🪬</div>
       <div class="panel center">
         <h2 style="color:var(--green)">The ward is sealed</h2>
         <div class="sub">\${this.layers} layer\${this.layers===1?'':'s'}, and the stone goes quiet.</div>
       </div>\`);
  },

  /* Only ever offered before the first riddle — once a layer is cut, the way
     out is \`sealNow\`, which keeps it. So there is nothing to zero here: a
     \`this.layers=0\` on this line would look like the careful thing to write and
     would in fact be unreachable, which is worse than not writing it. */
  walkAway(){
    this.seal(
      \`<div class="center crest">🌀</div>
       <div class="panel center"><h2>You leave the stone unmarked</h2>
       <div class="sub">It keeps whatever it was going to give.</div></div>\`);
  },

  /* The room's one report. \`ward\` is what the SHELL adds to the run — the room
     never touches the run itself, which is the whole of the Room Contract. */
  seal(html){
    const n=this.layers;
    const topics=this.spec.qs.slice(0, this.at).map(x=>x.key);
    this.finish({ status:'cleared', quality:n?1:0, topics, yield:{},
                  ward:n, sigil:{layers:n, asked:this.at} },
      html + (n ? \`<div class="panel center" style="margin-top:10px">
           <div style="font-weight:800;line-height:1.9;color:#8ad4ff">🌀 +\${n} ward\${n===1?'':'s'} carried</div>
           <div class="small" style="margin-top:6px">Each halves one blow, whenever it lands.
             They ride with the haul — a fall loses them too.</div>
         </div>\` : ''));
  },
  ...
  finish(outcome, html){
    this.resolved=true;
    const out=document.getElementById('lockOut') || document.getElementById('lockBody');
    out.innerHTML=html+\`<button class="btn gold" id="sigilGo" style="margin-top:12px">Onward →</button>\`;
    document.getElementById('sigilGo').onclick=()=>{
      const d=this.done; this.done=null; if(d) d(outcome);
    };
    if(outcome.ward>0){ Sfx.coin(); Haptic.win(); }
    else { Sfx.bad ? Sfx.bad() : 0; Haptic.hurt(); }
  }
};
...
/* ------------------------------- the rumour ------------------------------- */
/* Rumours' UI half, and the cheapest room in the game to describe: a stranger
 * at a table who will tell you what the rest of the night asks about, for a cut
 * of the pot.
 *
 * The price is a fraction rather than a number, which is the whole design. A
 * knight sitting on four hundred pays four times what one sitting on a hundred
 * pays, so information costs what it is worth to the person buying it — and
 * folding is always free, because a room that can only be entered by spending
 * is a gate, and this game does not have those.
 */
const Rumour = {
  spec:null, ctx:null, done:null, resolved:false,
  CUT: 0.15,                              // of the pot, rounded, minimum one coin

  price(pot){ return pot<1 ? 0 : Math.max(1, Math.round(pot*this.CUT)); },
  ...
  /* What is left to hear: the hands after this room, in the order they come.
     Asked of the same peek() the fork's foresight uses, so the two can never
     disagree about what a room is going to be. */
  ahead(){
    const set=SETTINGS[this.ctx.setting] || Dungeon.set();
    const out=[];
    for(let d=this.ctx.depth+1; d<=(set.rooms||0); d++){
      const r=Dungeon.peek(d);
      if(r && r.name==='wager' && r.topic) out.push({depth:d, topic:r.topic});
    }
    return out;
  },

  offer(){
    const pot=this.pot(), cost=this.price(pot), left=this.ahead();
    const body = !left.length
      ? \`<div class="sub">…but there are no hands left tonight. Whatever they know,
          it is no use to you now.</div>\`
      : \`<div class="sub">There are <b>\${left.length}</b> hand\${left.length===1?'':'s'} still
          to come. For a cut of the pot the stranger will tell you what each one asks about —
          which is worth something only if it changes what you would have staked.</div>\`;
    document.getElementById('lockBody').innerHTML=\`
      <div class="center crest">🗣️</div>
      <div class="panel">
        <div class="row" style="justify-content:space-between;display:flex">
          <span class="pill">🗣️ Table \${this.ctx.depth}</span>
          <span class="pill">🎒 pot \${pot}</span>
        </div>
        <h2 style="font-size:18px;margin-top:8px">A stranger with something to say</h2>
        \${body}
      </div>
      \${left.length && cost ? \`<button class="btn gold" onclick="Rumour.buy()">
          🗣️ Hear it — \${cost} gold
          <span style="font-size:12px;font-weight:700;margin-left:6px;color:rgba(40,26,4,.72)"
          >a \${Math.round(this.CUT*100)}% cut of what you are carrying</span>
        </button>\` : ''}
      <div class="center" style="margin-top:6px">
        <span class="kbd" onclick="Rumour.walkAway()">\${left.length && cost ? 'Buy nothing' : 'Move along'}</span>
      </div>
      <div style="height:16px"></div>\`;
  },

  buy(){
    const cost=this.price(this.pot());
    if(cost<1 || cost>this.pot()) return;
    this.ctx.run.unbanked.gold -= cost;
    const left=this.ahead();
    Sfx.coin(0); Haptic.tap();
    this.finish({ status:'cleared', quality:1, topics:[], yield:{},
                  rumour:{paid:cost, heard:left.length} },
      \`<div class="center crest">🗣️</div>
       <div class="panel">
         <h2 style="font-size:18px">“Since you are paying.”</h2>
         <div class="sub">What the rest of the night will ask you about.</div>
         <hr>
         \${left.map(x=>\`<div style="display:flex;justify-content:space-between;
             font-size:14px;font-weight:700;line-height:2">
             <span>Table \${x.depth}</span>
             <span style="color:var(--gold)">\${TOPIC_LABEL[x.topic]||x.topic}</span>
           </div>\`).join('')}
         <hr>
         <div class="small">Knowing the topic is not knowing the answer — but it is knowing
           whether the big stake is the one to take.</div>
       </div>\`);
  },

  walkAway(){
    this.finish({ status:'cleared', quality:0, topics:[], yield:{},
                  rumour:{paid:0, heard:0} },
      \`<div class="center crest">🗣️</div>
       <div class="panel center"><h2>You let them talk to somebody else</h2>
       <div class="sub">The pot is exactly where you left it.</div></div>\`);
  },
  ...
  finish(outcome, html){
    this.resolved=true;
    const out=document.getElementById('lockOut') || document.getElementById('lockBody');
    out.innerHTML=html+\`<button class="btn gold" id="rumourGo" style="margin-top:12px">Onward →</button>\`;
    document.getElementById('rumourGo').onclick=()=>{
      const d=this.done; this.done=null; if(d) d(outcome);
    };
  }
};
...
/* -------------------------------- the opening ----------------------------- */
/* Why arithmetic hurts a dragon, in three sentences. A game about practice
   cannot open on screens of invented history — nobody reads it and it delays
   the first question — but the conceit does need an answer, or a player who
   thinks the maths is arbitrary will treat it as arbitrary. Twenty seconds,
   skippable in one tap, and kept in the Tome to be read again. */
const OPENING = [
  {ic:'⚖️', t:'The realm balances',
   b:'The Eigenrealm holds together because its quantities balance — every length, every angle, every rate agreeing with every other.'},
  {ic:'🐉', t:'Where it stops, something goes wrong',
   b:'Where they stop balancing, something crawls out of the gap: a slime that will not sum, a wyrm whose determinant has collapsed.'},
  {ic:'⚔️', t:'A blade that resolves',
   b:'A knight’s blade does not cut. It <b>resolves</b> — and that is why the only weapon that works down here is the mathematics itself.'}
];
const Opening = {
  i:0, after:null,
  /* Show the cards, then run \`after\`. Already seen? Straight through. */
  play(after){
    this.after=after;
    if(Game.s && Game.s.seenOpening){ this.done(); return; }
    this.i=0; UI.go('s-opening'); this.card();
  },
  // Re-read from the Tome: the same cards, ending back where you were.
  reread(){ this.after=()=>UI.go('s-tome'); this.i=0; UI.go('s-opening'); this.card(); },
  card(){
    const c=OPENING[this.i], last=this.i===OPENING.length-1;
    document.getElementById('openBody').innerHTML=\`
      <div class="center crest">\${c.ic}</div>
      <div class="panel center">
        <div class="row" style="justify-content:center;gap:6px;margin-bottom:10px">
          \${OPENING.map((x,j)=>\`<span class="rp \${j<this.i?'done':j===this.i?'now':''}"></span>\`).join('')}
        </div>
        <h1 style="font-size:19px;color:var(--gold)">\${c.t}</h1>
        <div class="sub" style="margin-top:10px;font-size:15px;line-height:1.65">\${c.b}</div>
      </div>
      <button class="btn gold" id="openNext">\${last?'Take up the blade →':'Go on →'}</button>
      <div class="center" style="margin-top:8px">
        <span class="kbd" id="openSkip">Skip the tale</span>
      </div>
      <div style="height:20px"></div>\`;
    document.getElementById('openNext').onclick=()=>{
      if(last) this.done(); else { this.i++; this.card(); }
    };
    document.getElementById('openSkip').onclick=()=>this.done();
  },
  done(){
    if(Game.s && !Game.s.seenOpening){ Game.s.seenOpening=1; Game.save(); }
    const go=this.after; this.after=null;
    if(go) go(); else UI.go('s-map');
  }
};

/* ----------------------------- the quartermaster -------------------------- */
/* The only person at the Keep when a new knight arrives, and the shortest path
   from a cold start to a first fight. She hands over a blade and points at the
   cellar stairs; the map stays shut until the knight comes back up them. */
const Quartermaster = {
  open(){
    UI.go('s-opening');
    document.getElementById('openBody').innerHTML=\`
      <div class="center crest">🗝️</div>
      <div class="panel">
        <h2 style="font-size:18px;color:var(--gold)">The quartermaster</h2>
        <div class="sub" style="margin-top:8px;font-size:15px;line-height:1.65">
          “New, are you. Everyone starts the same way — down the stairs, three rooms,
          nothing in them that can finish you. There is a chest in the middle; here are
          picks for it.<br><br>
          Come back up and the realms are yours to walk.”
        </div>
        <hr>
        <div class="row" style="flex-wrap:wrap;justify-content:center">
          <span class="pill">🗡️ Rusted Trainer</span>
          <span class="pill">🧪 Healing Draught ×2</span>
          <span class="pill">🔧 Lockpick ×3</span>
        </div>
      </div>
      <button class="btn gold" id="qmGo">🕯️ Down to the cellar →</button>
      <div style="height:20px"></div>\`;
    document.getElementById('qmGo').onclick=()=>Dungeon.descend('cellar');
  }
};

/* ------------------------------ meeting a room ---------------------------- */
/* The first time a knight meets a kind of room, the game says what the verb is
   before asking them to perform it. Once each, per knight, and never again —
   this is onboarding, not a tooltip that nags forever. */
const ROOM_INTRO = {
  ...
  rumour: {ic:'🗣️', t:'A stranger with something to say',
    b:'This table sells information rather than a hand. For a cut of the pot you are carrying, you hear what the remaining hands will ask about — worth paying for only when it would change what you stake. Walking past costs nothing.'},
  copy: {ic:'✒️', t:'A scriptorium',
    b:'Four lines to copy, and every one you get right is worth pages. A line you get wrong does not stop you and takes nothing back — it costs precision, and every line after it is worth that much less. A slip on the first line is dearer than the same slip on the last.'},
  hold: {ic:'⚓', t:'A cargo hold',

    b:'Gold turned into freight grows every league you keep sailing — but it is not insured. A wreck takes cargo whole, where Dead reckoning would have carried part of the pot ashore with you. Interest against insurance, and both numbers are on the buttons.'},
  forage: {ic:'🌿', t:'A thicket',
    b:'Two decisions, not one. Every riddle you answer clears another square of basket — a wrong one ends the gathering, but you keep the room you have already made. Then you choose what to carry: each plant takes its own space, and the dearest is not always worth the room it wants.'},
  sigil: {ic:'🌀', t:'A ward-stone',
    b:'No foe, and nothing here pays in gold. Every riddle you answer cuts a layer into the stone, and every layer halves one blow for the rest of the descent. Press on for another and you stake the last one you cut — miss, and it cracks with it. The first is free.'},
...
const RoomIntro = {
  needed(kindName){
    return !!(ROOM_INTRO[kindName] && Game.s && !(Game.s.metRoom||{})[kindName]);
  },
  show(kindName, go){
    const c=ROOM_INTRO[kindName];
    if(!c){ go(); return; }
    Game.s.metRoom=Game.s.metRoom||{}; Game.s.metRoom[kindName]=1; Game.save();
    UI.go('s-opening');
    document.getElementById('openBody').innerHTML=\`
      <div class="center crest">\${c.ic}</div>
      <div class="panel center">
      ...
      <button class="btn gold" id="introGo">Ready →</button>
      <div style="height:20px"></div>\`;
    document.getElementById('introGo').onclick=go;
  }
};

/* ------------------------------- the Dungeon ------------------------------ */
/* Policy #2 over the shared engine — the endless Arena was #1. The Arena keeps
   every coin whether you walk out or are carried; the Deep does the opposite,
   and that difference is the whole game here. What you win stays UNBANKED,
   riding on the run: leave and it is yours, fall and it is gone. So every
   cleared room is a fork — press deeper for a bigger pot, or climb out now. */
const Dungeon = {
  active:false, run:null, cur:null,
  realm:{nm:'The Deep', col:'#6a44b0', sky:['#160f22','#080510'], pool:null,
    bg:{key:'deep', seed:53, sky:['#1c1430','#120b22','#070410'],
        far:'towers', farCol:'#241a3a', near:'arcs', nearCol:'#150e28',
        ground:['#241636','#0b0616'], rim:'rgba(180,139,236,.2)', haze:'rgba(24,16,44,.84)',
        glow:'#d9c4ff', weather:'dust', accent:'#b48bec'}},

  // Opened once the campaign's first realm boss has fallen — reachable well
  // before the Arena, since the Deep is the core loop, not the victory lap.
  unlocked(){ return !!(Game.s && Game.s.cleared['0:'+(REALMS[0].foes.length-1)]); },
  ...
  /* And the Wilds one later again. Its fork asks about a stretch rather than a
     room, which is only a question worth asking of a knight who has already
     learned to ask the simpler one. */
  wildsOpen(){ return !!(Game.s && REALMS[4] && Game.s.cleared['4:'+(REALMS[4].foes.length-1)]); },
  /* And the Sea one later again. It is the only place that asks a knight to
     price interest against insurance, which wants a purse worth insuring. */
  seaOpen(){ return !!(Game.s && REALMS[5] && Game.s.cleared['5:'+(REALMS[5].foes.length-1)]); },
  /* And the Library last of the seven. Its pressure is read off the mastery
     model, which needs a knight who has one worth reading. */
  libraryOpen(){ return !!(Game.s && REALMS[6] && Game.s.cleared['6:'+(REALMS[6].foes.length-1)]); },
  ...
  /* Foresight, and the reason it is rationed by the descent rather than by the
     ...
     it comes round is not one. Charges are taken once, at the mouth. */
  armForesight(){
    this.forkLeft={};
    for(const a of Loadout.chosen())
  ...
  /* What the next room is, built exactly as nextRoom would build it — the same
     seed, the same roll — so what you are shown is what you will meet. It is
     rebuilt rather than remembered, which keeps foresight honest: there is no
     second copy of the layout that could disagree with the real one. */
  /* What lies at a depth, worked out the way nextRoom works it out — the same
     seed, the same roll — so what foresight shows is what you will meet. Only
     the cheap half of the room is rebuilt: the kind, the foe a fight would
     bring, and the topic a table would ask about. Building the whole room here
     would draw questions nobody is going to answer, and would move the mastery
     scheduler's no-repeat cursor while it did it. */
  /* Which kind of room stands at a depth. One function, called by the shell
     when it builds a room and by foresight when it reads one, because there is
     no worse bug in this game than a reading that disagrees with the room — and
     two hand-kept copies of a cascade is precisely how that happens. Must be
     called with the room seed live: the rolls are part of the seeded stream.

     A setting with a plan lays its rooms out by hand. Otherwise depth one is
     always a fight, and past that each kind gets a roll in turn. */
  roomKindAt(depth, set){
    if(set.plan) return set.plan[depth-1];
    if(depth<2) return 'monster';
    if(R.chance(set.lockChance)) return 'lock';
    if(R.chance(set.seamChance||0)) return 'seam';
    if(R.chance(set.sigilChance||0)) return 'sigil';
    if(R.chance(set.forageChance||0)) return 'forage';
    if(R.chance(set.holdChance||0)) return 'hold';
    if(R.chance(set.copyChance||0)) return 'copy';
    return 'monster';

  },

  /* Whether the league at \`depth\` is running rough, as a multiplier on the foe
     that stands in it. 1 everywhere but the Sea.

     It is drawn from a stream of ITS OWN — mixed from the run seed with a
     different constant than the room cascade uses — for two reasons. The
     weather has to be readable from anywhere without disturbing what kind of
     room is there, and it has to read the same way twice however it is reached,
     because a forecast that disagreed with the weather would be no forecast.

     The save/restore around the seed matters: this is called from inside
     \`monster.build\`, which the shell has already seeded, and a bare R.unseed()
     there would drop the room's own stream on the floor mid-build. */
  WEATHER_MIX: 2246822519,
  rough(depth, set, seed){
    const c=(set || this.set()).waves;
    if(!c.squallChance) return 1;
    const s = (seed===undefined ? (this.run && this.run.seed) : seed) || 1;
    const prev = R._r;
    R.seed(((s ^ (depth * this.WEATHER_MIX)) >>> 0) || 1);
    const rough = R.chance(c.squallChance);
    R._r = prev;
    return rough ? c.squallMul : 1;
  },
  isRough(depth, set, seed){ return this.rough(depth, set, seed) !== 1; },

  /* The Library's pressure, in one number: how many difficulty tiers the stacks
     have added by this shelf. 0 everywhere else.

     Loremaster divides it, which is the whole reason this setting is that
     skill's home. A knight who has kept their reading warm walks twice as far
     along the shelf before the material gets hard — and one who has let it go
     cold meets the top tier almost at once. It is read live off the same faded
     mastery every other Passage effect is priced on, so letting the strand cool
     takes it away again. */
  stacks(depth, set){
    const S = set || this.set();
    if(!S.stackEvery) return 0;
    const d = depth===undefined ? (this.run ? this.run.depth : 1) : depth;
    return Math.floor(Math.max(0, d-1) / (S.stackEvery * Passage.memory()));
  },


  peek(depth){
    const set=this.set();
    if(depth<1 || (set.rooms && depth>set.rooms)) return null;
    R.seed(((this.run.seed ^ (depth * 2654435761)) >>> 0) || 1);
    const name = this.roomKindAt(depth, set);
    let foe=null, topic=null;
    if(name==='monster') foe=WaveEngine.foe(depth, set.waves, this.rough(depth, set));
    if(name==='wager'){
      // The same draw RoomKinds.wager.build makes, and only the draw — \`pure\`
      // keeps it out of the no-repeat cursor, so peeking cannot change what the
      // room turns out to be.
      const pool=WaveEngine.pool(Game.s.topicStats);
      const prefer=settingTopics(set) ||
                   (REALMS[Game.s.realm] ? REALMS[Game.s.realm].pool : null);
      topic=Mastery.pick(pool, prefer, true);
    }
    R.unseed();
    // The weather rides alongside the kind, because it is true of a league
    // whatever is standing in it.
    return {name, foe, topic, rough:this.isRough(depth, set)};
  },

  /* Spend a Farsight charge. The reading names the room and, for a fight, how
     it stands against the health you are actually carrying — a number the
     player would otherwise have to hold in their head. */
  scry(){
    if(!this.active || this.scried) return;
    if(this.forkCharges('farsight')<1){ UI.toast('No Farsight left this descent.'); return; }
    this.forkLeft.farsight--;
    const next=this.peek(this.run.depth+1);
    let line;
    if(next.name==='monster'){
      const f=next.foe, hp=Game.s.hp;
      // How many blows it can land before you fall, at its own attack rate.
      const per=Math.max(1, f.atk - Game.armor().def);
      const blows=Math.floor(hp/per);
      const read = blows<=3 ? 'it would take you apart' : blows<=6 ? 'it would hurt' : 'you can take it';
      line = \`<b>\${f.nm}</b>\${f.boss?' — a champion':''}. \${f.hp} health, striking for about \${f.atk}.
    ...
    } else if(next.name==='rumour'){
      line = \`A <b>stranger</b> selling what they know. No foe, and nothing at risk but a cut.\`;
    } else if(next.name==='sigil'){
      line = \`A <b>ward-stone</b>. No foe — you carry \${this.wards()} ward\${this.wards()===1?'':'s'} already.\`;
    } else if(next.name==='forage'){
      line = \`A <b>thicket</b>. No foe — only what you can carry out of it is at stake.\`;
    } else if(next.name==='hold'){
      line = \`A <b>cargo hold</b>. No foe — a chance to turn the pot into freight.\`;
    } else if(next.name==='copy'){
      line = \`A <b>scriptorium</b>. No foe — four lines to copy, and precision to lose.\`;
  ...
  takeSighting(){
    ...
    for(let d=from; d<from+this.SIGHT_RANGE; d++){
      ...
      /* "Pays" means a room that gives you something and cannot kill you doing
         it. A ward-stone counts: on a climb, where the thing that kills you is
         the accumulated blows rather than any one foe, a stone is worth more
         than a chest — so leaving it out would have Sighting quietly walk a
         climber past the best room on the ridge. */
      if(!pays && (r.name==='lock' || r.name==='seam' || r.name==='sigil'
                || r.name==='forage' || r.name==='hold' || r.name==='copy')) pays=at;
  ...
  /* Tracking. It reads a SHAPE rather than a place, which changes how long it
     lasts: a period does not expire, and neither does the fact that you have
     learned to read this ground. What does change is where in the turn you are
     standing, so the reading is recomputed at every fork rather than stored —
     and it runs out after one full turn of the swell, by which time a knight
     who was going to use it has used it.

     Like Rumours it covers a stretch rather than a room, because the question
     it answers is about a stretch. Unlike Rumours it is silent about what is in
     that stretch: it names the rhythm, and nothing at all about the contents. */
  trackedHere(){ return !!(this.tracked && this.run && this.tracked.through > this.run.depth); },

  takeTracking(){
    if(!this.active || this.trackedHere()) return;
    const set=this.set();
    if(!set.waves.swell){ UI.toast('Nothing here circles.'); return; }
    if(this.forkCharges('tracking')<1){ UI.toast('The trail tells you nothing more.'); return; }
    this.forkLeft.tracking--;
    const p=set.waves.swellEvery;
    this.tracked={ from:this.run.depth, through:this.run.depth+p };
    Sfx.coin(0); Haptic.tap();
    this.fork(this.lastOutcome||{});
  },

  /* Where the swell stands at the room you are about to walk into, said in the
     two numbers a decision turns on: how far to the next easing, and how far to
     the next crest. The strength ratio is quoted too, because "a swell" means
     nothing without knowing whether it is worth waiting out.

     Everything here is derived from the curve rather than remembered, so a
     reading can never disagree with the foes it describes — the same rule the
     rest of foresight is built on. */
  trackingLine(){
    const set=this.set(), c=set.waves, p=c.swellEvery;
    const next=this.run.depth+1;
    // Where the crest and the trough of the turn containing \`next\` sit, in room
    // numbers: the swell is 1 + A·sin(2π(n−1)/P), so it peaks a quarter of a
    // turn in and bottoms three quarters in.
    const ahead=(mark)=>{
      const d=Math.round(mark*p - (next-1));
      return ((d % p) + p) % p;                     // rooms from \`next\` to that mark
    };
    const toCrest=ahead(0.25), toTrough=ahead(0.75);
    const at=WaveEngine.swell(next, c);
    const span=(n)=>\`<b>\${n}</b> room\${n===1?'':'s'} on\`;
    const ratio=Math.round(((1+c.swell)/(1-c.swell) - 1)*100);
    /* Four states, not two. Standing ON the crest is not "walking into it" —
       the worst room is the one under your feet and everything after it is
       easier, which is the opposite advice. The turn's two ends each get their
       own sentence for that reason. */
    let where;
    if(toCrest===0)       where='The very next room is the crest of it — from there the ground eases.';
    else if(toTrough===0) where='The very next room is the low of the turn — after it the ground builds again.';
    else if(toCrest<toTrough) where=\`You are walking into it: it worsens to a crest \${span(toCrest)}, then eases.\`;
    else                  where=\`You are past the crest: it eases to the low \${span(toTrough)}, then builds again.\`;
    return \`The ground turns on a <b>\${p}</b>-room round. \${where}\` +
      \` Right now they are running <b>\${Math.round((at-1)*100)}%</b> off the middle, and a crest is
        about <b>\${ratio}%</b> harder than a low.\`;
  },

  /* Weather-reading. It covers a stretch, like Rumours and Tracking, and for
     the same reason: what it is about is a run of leagues rather than a room.
     The span is fixed rather than tied to a period, because the weather has no
     period — that is the whole difference between this place and the Wilds.

     Like Tracking, the reading is recomputed at every fork rather than stored,
     so what it says can never drift out of step with the water it describes. */
  FORECAST_SPAN: 5,
  forecastHere(){ return !!(this.forecast && this.run && this.forecast.through > this.run.depth); },

  readWeather(){
    if(!this.active || this.forecastHere()) return;
    const set=this.set();
    if(!set.waves.squallChance){ UI.toast('Nothing is drawn here.'); return; }
    if(this.forkCharges('weather')<1){ UI.toast('The sky tells you nothing more.'); return; }
    this.forkLeft.weather--;
    this.forecast={ from:this.run.depth, through:this.run.depth+this.FORECAST_SPAN };
    Sfx.coin(0); Haptic.tap();
    this.fork(this.lastOutcome||{});
  },

  /* The forecast itself: the leagues ahead, in order, marked calm or rough, and
     the two numbers a decision turns on — how far to the first blow, and how
     much of the stretch is weather. Everything is read off the same function
     the shell builds rooms with, so a forecast cannot disagree with the sea. */
  forecastLine(){
    const set=this.set(), from=this.run.depth+1;
    const marks=[]; let firstRough=0, rough=0;
    for(let i=0;i<this.FORECAST_SPAN;i++){
      const d=from+i;
      if(set.rooms && d>set.rooms) break;
      const bad=this.isRough(d, set);
      if(bad){ rough++; if(!firstRough) firstRough=i+1; }
      marks.push(bad ? '<b style="color:var(--red)">rough</b>' : 'calm');
    }
    if(!marks.length) return 'Nothing but the harbour ahead.';
    const mul=Math.round((set.waves.squallMul-1)*100);
    let line=\`The next <b>\${marks.length}</b> leagues: \${marks.join(' · ')}.\`;
    if(!rough) line += ' Clear water the whole way.';
    else {
      line += \` <b>\${rough}</b> of them are weather, and a rough league hits about
                <b>\${mul}%</b> harder.\`;
      line += firstRough===1 ? ' It starts with the very next one.'
                             : \` The first is <b>\${firstRough}</b> leagues on.\`;
    }
    return line;
  },

  /* Rumours. Where Farsight spends itself on one room and is gone by the time
     you stand in it, this buys a stretch — and so it has to last the stretch,
     or it would be a worse Farsight sold at the same price. \`heard.through\` is
     the last depth it covers, and the reading stays on the fork until that
     depth is behind you. */
  RUMOUR_SPAN: 3,
  heardHere(){ return !!(this.heard && this.run && this.heard.through > this.run.depth); },

  hearRumours(){
    if(!this.active || this.heardHere()) return;
    if(this.forkCharges('rumours')<1){ UI.toast('Nobody else is talking.'); return; }
    this.forkLeft.rumours--;
    const from=this.run.depth+1, rooms=[];
    for(let d=from; d<from+this.RUMOUR_SPAN; d++){
      const r=this.peek(d);
      if(r) rooms.push(r);
    }
    this.heard={ through:from+rooms.length-1, from, line:this.rumourLine(rooms) };
    Sfx.coin(0); Haptic.tap();
    this.fork(this.lastOutcome||{});
  },

  /* The reading itself: counts, and the two numbers a decision actually turns
     on — the total health standing between you and the far end, and the worst
     single blow you would have to take on the way. Deliberately silent about
     the order, because the order is what Farsight sells. */
  rumourLine(rooms){
    if(!rooms.length) return 'Nothing but the way out. This is the last of it.';
    const fights=rooms.filter(r=>r.name==='monster');
    const champ=fights.some(r=>r.foe && r.foe.boss);
    const hp=fights.reduce((n,r)=>n+r.foe.hp, 0);
    const worst=fights.reduce((n,r)=>Math.max(n, r.foe.atk), 0);
    const others={};
    for(const r of rooms) if(r.name!=='monster') others[r.name]=(others[r.name]||0)+1;
    const NAME={lock:['a chest','chests'], seam:['a seam','seams'],
                wager:['a table','tables'], rumour:['a rumour','rumours'],
                sigil:['a ward-stone','ward-stones'],
                forage:['a thicket','thickets'],
                hold:['a hold','holds'],
                copy:['a scriptorium','scriptoria']};

    const bits=[];
    if(fights.length) bits.push(\`<b>\${fights.length}</b> fight\${fights.length===1?'':'s'}\`);
    for(const k in others){
      const n=others[k], nm=NAME[k]||[k,k];
      bits.push(n===1 ? \`<b>\${nm[0]}</b>\` : \`<b>\${n}</b> \${nm[1]}\`);
    }
    const span = rooms.length===1 ? 'The next room holds'
               : \`The next <b>\${rooms.length}</b> rooms hold\`;
    let line = \`\${span} \${bits.join(' and ')}.\`;
    if(fights.length){
      const per=Math.max(1, worst - Game.armor().def);
      line += \` <b>\${hp}</b> health between you and the far end, and the hardest
                hitter among them lands about <b>\${per}</b> a blow.\`;
      if(champ) line += ' One of them is a <b>champion</b>.';
    }
    return line;
  },

  /* Wards cut at the ward-stones and not yet spent. Six is two full stones —
     enough that a good run feels armoured, short of the point where blows stop
     landing at all. Foresight is rationed per descent for the same reason. */
  WARD_CAP:6,
  wards(){ return (this.active && this.run && this.run.wards) || 0; },

  /* Spend one on a blow. Called from Battle, after the beast and after a raised
     Ward, so the two things a player chose to have are always used before the
     thing that was banked for them. */
  spendSigil(dmg){
    if(this.wards()<1) return dmg;
    this.run.wards--;
    Anim.float(560, H-260, 'THE WARD', '#b48bec');
    setTimeout(()=>Battle.renderPowers(), 60);
  ...
  descend(setting){
    ...
    this.run = { setting:key, seed, depth:from, paid:stake, from, wards:0, cargo:0,
  ...
  checkpoint(done){
    Game.s.run = { seed:this.run.seed, done, setting:this.run.setting,
                   ...
                   wards:this.run.wards||0,     // ward-stone layers still uncracked
                   cargo:Math.round(this.run.cargo||0),   // freight in the hold
  ...
  resume(){
    ...
    if(!r) return;
    /* Wards ARE carried across a reload, where a spent foresight charge is not.
       The difference is what each one is: a charge is a decision you get to
       make again, and re-arming it costs the player nothing they earned. A ward
       was cut out of three riddles answered correctly, so dropping it on a
       reload would take back work — and keeping it is the same rule the
       unbanked pot already follows. */
    this.run = { setting:r.setting||'deep', seed:r.seed, depth:r.done||0,
                 paid:r.paid||0, from:r.from||0, wards:r.wards||0, cargo:r.cargo||0,
                 unbanked:{ gold:r.gold||0, xp:r.xp||0 } };
    this.active = true;
    this.realm.pool = WaveEngine.pool(Game.s.topicStats);
    // A resumed descent gets its foresight back. The checkpoint is deliberately
    // {seed, depth, bank, health} and nothing else, so spent charges are not
    // carried across a reload — the alternative is a field in every save for a
    // thing that only matters for the next thirty seconds.
    this.armForesight();
    Game.s.maxHp = Game.maxHp();
  ...
  // only live while the room is being BUILT — combat rolls afterwards stay
  // genuinely random, so you cannot memorise a fight, only its foe.
  nextRoom(){
    this.run.depth++;
    ...
    Game.s.hp = Math.min(Game.s.hp, Game.s.maxHp);
    R.seed(((this.run.seed ^ (depth * 2654435761)) >>> 0) || 1);
    // The cellar puts a chest in the middle so a first run is guaranteed to
    // meet one; elsewhere the kind is a seeded roll, so a run replays
    // identically — and foresight reads it through the same function.
    const name = this.roomKindAt(depth, set);
    const kind = RoomKinds[name] || RoomKinds.monster;
    const spec = kind.build(R, depth, set);
    R.unseed();
    this.cur = { kind, spec };
    const ctx = { run:this.run, depth, setting:set.key,
                  rng:R, canDie:set.canDie, resume:null };
    const go = () => kind.enter(spec, ctx, outcome => this.resolve(outcome));
    // Say what this kind of room asks of you, the first time you meet one.
    if(RoomIntro.needed(name)) RoomIntro.show(name, go); else go();
  },

  // The shell owns everything between rooms: staging the yield, the fork,
  // banking, and ending the run on death.
  resolve(outcome){
    const set=this.set();
    if(outcome.status!=='cleared' && !set.canDie){
      // A setting that cannot kill you: the foe breaks off instead. You keep
      // what you carried and the run goes on, because a first run that ends in
      // a death screen has taught the player only that they should not have
      // tried. They are put back on their feet, not healed to full.
      Game.s.hp=Math.max(Math.round(Game.s.maxHp*.5), Math.round(Game.s.hp));
      outcome={ status:'cleared', quality:0, topics:outcome.topics||[], yield:{}, brokeOff:true };
    }
    if(outcome.status==='cleared'){
      const y=outcome.yield||{};
      this.run.unbanked.gold += y.gold||0;
      this.run.unbanked.xp   += y.xp||0;
      // A ward-stone pays in protection rather than coin, and the shell applies
      // it for the same reason it applies the gold: the room reports, the run
      // records. Capped so a lucky stretch of stones cannot make a knight
      // effectively unhittable for the rest of a descent.
      if(outcome.ward) this.run.wards = Math.min(this.WARD_CAP, (this.run.wards||0) + outcome.ward);
      /* Freight compounds for every league CLEARED, not every league entered.
         That distinction is the whole of it: \`resume\` re-enters the room you
         left, so growing on entry would let a knight quit and come back to
         compound the hold for nothing, over and over. A cleared league is
         banked in the checkpoint and cannot be re-cleared, so it can only be
         counted once. Applied before the stow below, so freight put aboard this
         league starts growing next league rather than the moment it lands. */
      if(this.run.cargo>0) this.run.cargo *= (1 + Hold.GROWTH);
      /* Cargo is a CONVERSION rather than a yield: it comes out of the pot
         rather than out of the room, so the shell moves it across instead of
         adding it. Clamped to what is actually in the pot, because the room
         quotes a share of a number that nothing else may have changed since. */
      if(outcome.hold && outcome.hold.stowed){
        const take=Math.min(Math.floor(outcome.hold.stowed), Math.floor(this.run.unbanked.gold));
        if(take>0){
          this.run.unbanked.gold -= take;
          this.run.cargo = (this.run.cargo||0) + take;
        }
      }
      // The Sanctum pays in essence for every room you get through, banked with
      // the rest — so a fall there costs the essence too.
      if(set.essencePerRoom)
        this.run.unbanked.essence = (this.run.unbanked.essence||0) + set.essencePerRoom;
      this.checkpoint(this.run.depth);   // this depth is behind you now
      // A run of fixed length is over when its last room falls — there is no
      // fork to offer, so it banks itself and walks you back up.
      if(set.rooms && this.run.depth>=set.rooms) this.finishRun(outcome);
      else this.fork(outcome);
    } else {
      this.died(outcome);        // canDie run — a fall forfeits the unbanked pot
    }
  },

  /* The end of a fixed-length run. Everything carried is banked — you walked
     out of the last room, which is the same thing as climbing the stairs. */
  finishRun(outcome){
    const u={gold:this.run.unbanked.gold + Math.floor(this.run.cargo||0),
             xp:this.run.unbanked.xp,
    ...
    Game.s.hp=Math.min(Game.s.hp, Game.s.maxHp);
    this.clearRun();
    Game.s.firstRun=1;                 // the cellar is behind them; the map opens
    Game.save();
    UI.go('s-result');
    document.getElementById('resultBody').innerHTML=\`
      <div class="center crest">🏰</div>
      ...
      <div style="height:20px"></div>\`;
    Haptic.win(); Sfx.win();
    if(ups) setTimeout(()=>Celebrate.banner('LEVEL '+Game.s.lvl,'Maximum health now '+Game.s.maxHp,'var(--gold)',()=>Sfx.level()), 600);
    Titles.check();
  },
  ...
  fork(outcome){
    ...
    if(sp.kind==='lock'){
      ...
      icon = opened?'🎁':'🧰';
      line = (opened ? 'The chest gives up its hoard. ' : 'The chest stays shut. ') + PASSAGE;
    } else if(sp.kind==='seam'){
      const ore = (outcome.seam && outcome.seam.ore) || 0;
      icon = ore?'🪨':'⛰️';
      line = (ore ? \`The seam gives up \${ore} ore. \` : 'The seam keeps what it has. ') + PASSAGE;
    } else if(sp.kind==='copy'){
      const c = outcome.copy || {pages:0, slips:0};
      icon = c.pages ? (c.slips?'📜':'📜✨') : '✒️';
      line = (c.pages ? \`\${c.pages} pages copied\${c.slips?\`, \${c.slips} line\${c.slips===1?'':'s'} astray\`:' without a slip'}. \`
                      : 'The passage stays uncopied. ') + PASSAGE;
    } else if(sp.kind==='hold'){

      const h = outcome.hold || {stowed:0};
      icon = h.stowed?'📦':'⚓';
      line = (h.stowed ? \`\${h.stowed} gold goes into the hold. \` : 'The hold stays empty. ') + PASSAGE;
    } else if(sp.kind==='forage'){
      const f = outcome.forage || {herbs:0};
      icon = f.herbs?'🧺':'🌿';
      line = (f.herbs ? \`You carry \${f.herbs} herbs out of the thicket. \` : 'The thicket keeps what it has. ') + PASSAGE;
    } else if(sp.kind==='sigil'){
      const n = (outcome.sigil && outcome.sigil.layers) || 0;
      icon = n?'🪬':'🌀';
      line = (n ? \`The ward holds at \${n} layer\${n===1?'':'s'}. \` : 'The stone stays blank. ') + PASSAGE;
    } else if(sp.kind==='rumour'){
      const r = outcome.rumour || {paid:0, heard:0};
      icon = r.paid?'🗣️':'🤫';
      line = (r.paid ? \`You paid \${r.paid} to hear what is coming. \`
                     : 'You let them talk to somebody else. ') + 'Another table waits.';
    ...
    document.getElementById('resultBody').innerHTML=\`
      ...
      <div class="panel center">
        ...
        <div style="font-size:15px;font-weight:800;line-height:1.8">
          ...
          <div style="color:var(--blue)">✦ \${u.xp} experience carried</div>
          \${u.essence?\`<div style="color:#b48bec">🜄 \${u.essence} essence carried</div>\`:''}
          \${this.wards()?\`<div style="color:#8ad4ff">🌀 \${this.wards()} ward\${this.wards()===1?'':'s'} carried</div>\`:''}
          \${this.run.cargo>=1?\`<div style="color:#7fc98a">📦 \${Math.floor(this.run.cargo)} in freight — uninsured</div>\`:''}
        </div>
        <hr>
  ...
  /* What Scrying puts on the fork. Three states, and each says something
     different: a reading you have paid for, an offer to pay for one, or — when
     the skill is not carried or not yet known — nothing at all, because an
     empty button advertising a skill you do not have is noise. */
  /* What foresight puts on the fork. Two abilities now, each in one of three
     states — a reading you have paid for, an offer to pay for one, or, when the
     skill is not carried or not yet known, nothing at all, because an empty
     button advertising a skill you do not have is noise.

     Written as a table because there are two of them and there will be four:
     the design places Tracking and Weather-reading here as well, and each is a
     line in this list rather than another branch to keep in step. */
  FORESIGHT:[
    {id:'farsight', ic:'🔭', col:'#6a44b0',
     offer:'Scry the passage below', spent:'Farsight spent for this descent',
     call:'Dungeon.scry()',
     reading(d){ return d.scried && d.scried.line; }},
    ...
    {id:'weather', ic:'🌦️', col:'#8ad4ff',
     offer:'Read the sky', spent:'The sky tells you nothing more',
     call:'Dungeon.readWeather()',
     reading(d){ return d.forecastHere() ? d.forecastLine() : null; },
     /* Same escape hatch Tracking has, for the same reason: in a place with no
        weather there is nothing to forecast, and selling a charge for that
        sentence would be a trap. */
     mute(d){ return d.set().waves.squallChance ? null
                : 'The sky here has nothing to say — nothing is drawn'; }},
    {id:'tracking', ic:'🐾', col:'#7fc98a',
     offer:'Read the ground', spent:'The trail tells you nothing more',
     call:'Dungeon.takeTracking()',
     reading(d){ return d.trackedHere() ? d.trackingLine() : null; },
     /* The one ability with a place where it has nothing to say. Reading the
        ground for a period only means something where there IS one; in the Deep
        or on the Summit the danger only climbs. Rather than sell a charge for
        that sentence, the fork says so and keeps the charge. */
     mute(d){ return d.set().waves.swell ? null : 'Nothing here circles — the ground only climbs'; }}
  ],

  foresightBlock(){
    const carried=new Set(Loadout.chosen().map(a=>a.id));
    return this.FORESIGHT.map(f=>{
      const said=f.reading(this);
      if(said) return \`<div class="panel" style="border-color:\${f.col}">
        <div class="row" style="display:flex;gap:8px;align-items:center">
          <span style="font-size:22px">\${f.ic}</span>
          <div class="small" style="flex:1;line-height:1.5">\${said}</div>
        </div></div>\`;
      if(!carried.has(f.id)) return '';
      const mute=f.mute && f.mute(this);
      if(mute) return \`<div class="center small" style="opacity:.6;margin:4px 0">
          \${f.ic} \${mute}</div>\`;
      const left=this.forkCharges(f.id);
      if(left<1) return \`<div class="center small" style="opacity:.6;margin:4px 0">
          \${f.ic} \${f.spent}</div>\`;
      return \`<button class="btn" style="border-color:\${f.col}"
          onclick="\${f.call}">\${f.ic} \${f.offer}
          <span style="font-size:12px;font-weight:700;color:var(--dim);margin-left:6px">\${left} left</span>
        </button>\`;
    }).join('');
  },

  // Bank the pot and return to the surface. \`midFight\` is true when the player
  // fled a room instead of clearing it — the loot is the same, the flavour isn't.
  leave(midFight){
    ...
    // Freight is sold the moment it reaches shore, at whatever it has grown to.
    const freight=Math.floor(this.run.cargo||0);
    Game.s.gold += u.gold + freight;
    if(u.essence) Game.s.mats.essence += u.essence;
    const ups = Game.gainXp(u.xp);
    Game.s.maxHp=Game.maxHp();
    Game.s.hp=Math.min(Game.s.hp, Game.s.maxHp);
  ...
  died(){
    /* Only a fight can end a run today, so \`cur\` is normally a monster room —
    ...
    /* Cargo goes down with the ship, whole. Dead reckoning is a knight finding
       their own way out with what they can carry; nobody swims a hold ashore,
       and that asymmetry is the entire decision the hold room asks. */
    const freight=Math.floor(this.run.cargo||0);
    const kept=Math.floor(pot * Passage.salvage());
    const lost=pot-kept+freight;
    this.active=false;
...
const UI = {
  ...
  refreshTitle(){
    ...
    /* The title card has three states and should look like it: nobody here
       yet, a knight safe at the Keep, or a knight parked in the middle of a
       cave. The third one has to say what is at stake, because the whole point
       of a saved run is that walking back into it is a decision. */
    const has=!!(k && Game.s);
    const pend=has ? Dungeon.pending() : null;
    const begin=document.getElementById('btnBegin'), cont=document.getElementById('btnContinue');
    const line=document.getElementById('titleState');
    cont.style.display = has ? '' : 'none';
    document.getElementById('resetRow').style.display = has ? '' : 'none';
    begin.textContent = has ? '⚔️ Start a new quest' : '⚔️ Begin the Quest';
    begin.className = has ? 'btn ghost' : 'btn gold';
    if(!has){
      cont.style.display='none';
      line.innerHTML='';
    } else if(pend){
      const set=SETTINGS[pend.setting||'deep']||SETTINGS.deep;
      cont.className='btn gold';
      cont.innerHTML=\`🕯️ Return to \${set.nm} — room \${(pend.done||0)+1}\`;
      cont.setAttribute('onclick','Dungeon.resume()');
      line.innerHTML=\`<div class="panel center" style="padding:9px">
        <div class="small">❤️ \${pend.hp} health · 🎒 <b class="coin">\${pend.gold||0} gold</b> carried and not yet banked</div></div>\`;
    } else {
      const st=Game.s.streak||{days:0};
      cont.className='btn gold';
      cont.innerHTML=\`🏰 Continue as \${Knights.esc(k.nm)}\`;
      cont.setAttribute('onclick',"UI.go('s-map')");
      line.innerHTML=\`<div class="panel center" style="padding:9px">
        <div class="small">Level \${Game.s.lvl} · 🪙 \${Game.s.gold} gold\${st.days>1?\` · 🔥 \${st.days} day streak\`:''}</div></div>\`;
    }
  },
  glint(){
  ...
  renderPrefs(){
    ...
    // The stamp says which build this is, and whether the gates were opened
    // rather than won — both things a bug report needs to carry.
    paintBuild();
  },
  renderMap(){
    if(!Game.s) return;
    ...
    if(!g.firstRun && !Dungeon.pending()) head+=\`
      <div class="node" style="border-color:var(--gold-dk)" onclick="Quartermaster.open()">
        <div class="ico" style="font-size:24px">🗝️</div>
        <div style="flex:1">
          <div class="nm">The quartermaster is waiting</div>
  ...
  renderShop(){
    ...
    out.push(\`</div><div class="panel"><h2>🌿 The Apothecary</h2>
      <div class="sub">Herbalism makes up <b>herbs</b>, gathered from thickets out in the
        Wilds and spent nowhere else. You hold <b>🌿 \${g.mats.herb}</b>. Most of this bench
        can be bought again; the satchel is made once.</div>\`);
    APOTHECARY.forEach(e=>{
      const done = e.owned && e.owned(g);
      const can = !done && g.mats.herb >= e.herb;
      out.push(\`<div class="item \${done?'equipped':''}">
        <div class="ic">\${e.ic}</div>
        <div style="flex:1"><div class="nm">\${e.nm}</div><div class="ds">\${e.ds}</div></div>
        \${done ? '<span class="pill">made</span>'
               : \`<button class="btn sm \${can?'gold':''}" style="width:auto"
                    onclick="Apothecary.brew('\${e.id}')">🌿 \${e.herb}</button>\`}
      </div>\`);
    });
    out.push(\`</div><div class="panel"><h2>📄 The Scriptorium</h2>
      <div class="sub">Scribing spends <b>pages</b>, copied in the Library and spent nowhere
        else. You hold <b>📄 \${g.mats.page}</b>. Binding sets a topic's clock back — it buys
        <b>recency</b>, never mastery. \${(() => {
          const cold = Scriptorium.coldest(12).length;
          return cold ? \`<b>\${cold}</b> topic\${cold===1?' has':'s have'} faded enough to be worth binding.\`
                      : 'Nothing has faded enough to be worth binding yet.';
        })()}</div>\`);
    SCRIPTORIUM.forEach(e=>{
      const can = g.mats.page >= e.page && Scriptorium.coldest(1).length > 0;
      out.push(\`<div class="item">
        <div class="ic">\${e.ic}</div>
        <div style="flex:1"><div class="nm">\${e.nm}</div><div class="ds">\${e.ds}</div></div>
        <button class="btn sm \${can?'gold':''}" style="width:auto"
          onclick="Scriptorium.bind('\${e.id}')">📄 \${e.page}</button>
      </div>\`);
    });
    /* Alchemy's bench. The three buttons are the teaching: they quote what each
       share would actually come back as, so a player sees the second half of a
       pile yield about half what the first half did without being told why. */
    {
      const from=Crucible.from, to=Crucible.to;
      const a=Crucible.mat(from), b=Crucible.mat(to);
      const src=Crucible.held(from);
      const band=['weak','steady','solid'][Crucible.tier()];
      const chip=(which,m)=>{
        const on = (which==='from' ? from : to) === m.id;
        const dead = which==='to' && m.id===from;
        return \`<button class="btn sm \${on?'gold':''}" style="width:auto;opacity:\${dead?0.35:1}"
                  \${dead?'disabled':''} onclick="Crucible.point('\${which}','\${m.id}')"
                  >\${m.ic} \${m.nm} ×\${Crucible.held(m.id)}</button>\`;
      };
      out.push(\`</div><div class="panel"><h2>⚗️ The Crucible</h2>
        <div class="sub">Alchemy turns one material into another. Nothing is gathered here —
          what it sells is <b>conversion</b>, and it is never free. The rate follows the
        ...
        <div class="ds" style="margin-bottom:4px">into</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px">
          \${MATERIALS.map(m=>chip('to',m)).join('')}</div>\`);
      if(src <= 0){
        out.push(\`<div class="ds">You hold no \${a.nm}. That comes from \${a.from}.</div>\`);
      } else {
        Crucible.SHARES.forEach(sh=>{
          ...
          out.push(\`<div class="item">
            ...
            <div style="flex:1"><div class="nm">\${sh.nm} — \${q.spend} \${a.nm}</div>
              <div class="ds">comes back as <b>\${q.get}</b> \${b.nm}.</div></div>
            <button class="btn sm \${can?'gold':''}" style="width:auto" \${can?'':'disabled'}
              onclick="Crucible.pour('\${from}','\${to}',\${q.spend})">\${b.ic} \${q.get}</button>
          </div>\`);
        });
      }
    }
  ...
  /* The loadout, and the point of it: every row says how many uses it grants
     AND why — the strand it draws on and how well that strand is known. A
     player who wants more Wards is told, on the same line, that the way to get
     them is to get better at vectors. */
  loadoutPanel(){
    const chosen=Loadout.chosen().map(a=>a.id);
    const rows=SKILL_ABILITIES.map(ab=>{
      const on=chosen.indexOf(ab.id)>=0, n=Loadout.charges(ab), band=Loadout.band(ab);
      const pct=Math.round(Loadout.reach(ab)*100);
      const col=Mastery.colour(Loadout.reach(ab));
      return \`<div class="item \${on?'equipped':''}" style="cursor:pointer" onclick="Loadout.toggle('\${ab.id}')">
        <div class="ic">\${ab.ic}</div>
        <div style="flex:1">
          <div class="nm">\${ab.nm}
            <span class="tag">\${n} use\${n===1?'':'s'}</span></div>
          <div class="ds">\${ab.ds}</div>
          <div class="ds" style="margin-top:3px">
            \${ab.skill} · draws on <b>\${ab.strand}</b> ·
            <span style="color:\${col}">\${band} \${pct}%</span>
          </div>
        </div>
        <span class="pill">\${on?'carried':'carry'}</span>
      </div>\`;
    }).join('');
    return \`<div class="panel"><h2>🕸️ Skills</h2>
      <div class="sub">Three carried into every fight. Uses are not bought — they come
        from how well you know the mathematics behind them, and refill each fight.
        <b>\${chosen.length}/\${Loadout.SLOTS}</b> carried.</div>
      \${rows}
  ...
  chronicleBody(rows){
    ...
    const bar=(k,v)=>{
      // The bar shows EFFECTIVE mastery — what you know now, not what you knew
      // the day you last practised — so the fade is something a player can see
      // rather than only feel in the questions they are given. Where the two
      // differ enough to matter, the gap is named rather than left mysterious.
      const eff=Mastery.eff(k), pct=Math.round(eff*100);
      const col=Mastery.colour(eff), due=Mastery.overdue(k);
      const faded = (v.m-eff) >= 0.08;
      return \`<div style="margin:6px 0">
        <div style="display:flex;justify-content:space-between;font-size:12px;gap:6px">
          ...
          <span style="color:\${col};white-space:nowrap">\${Mastery.label(eff)} \${pct}%
            \${faded?\`<span class="small" style="opacity:.7" title="It fades while you are away">↓ from \${Math.round(v.m*100)}%</span>\`:''}
  ...
  renderTome(){
    // The opening is skippable, so it has to live somewhere it can be found
    // again — this is that somewhere.
    document.getElementById('tomeList').innerHTML =
      \`<div class="panel" style="border-color:var(--gold-dk)">
         <h2 style="font-size:16px;color:var(--gold)">📜 Why a blade of mathematics</h2>
         <div class="sub" style="margin-top:6px;font-size:14px">The tale you were told at the gate, in three cards.</div>
         <button class="btn sm" style="width:auto;margin-top:8px" onclick="Opening.reread()">Read it again</button>
       </div>\` +
...
const Knights = {
  ...
  carry(id){
    // A code is a snapshot of a knight, and a descent in flight is not part of
    // it — carrying one mid-run would strand the unbanked haul on this device
    // with nothing to say so. A knight is finished or parked before they travel.
    if(Profiles.r && id===Profiles.r.active && Dungeon.pending()){
      UI.toast('Climb out of the Deep first — a descent cannot travel with you.');
      return;
    }
...
/* Which build is running, shown in Settings so a bug report can name it. The
   build writes build-info.json next to index.html and stamps the worker with
   the same commit; the service worker's cache serves it offline, so the number
   shown always matches the build actually installed. file:// and Scriptable
   have no such file, and read as a local build.

   The stamp is also the way in to the testing cheat below, so it is painted
   from a stored string rather than written once: the line has to be able to
   change when a knight is unlocked, and Settings can be opened again later. */
let BUILD_LINE='build local';
function paintBuild(){
  const el=document.getElementById('buildStamp');
  if(!el) return;
  el.textContent = BUILD_LINE + ' · save format v'+Codec.VER
    + (Game.s && Game.s.cheated ? ' · unlocked for testing' : '');
}
function stampBuild(){
  const el=document.getElementById('buildStamp');
  if(!el) return;
  el.onclick=()=>Cheats.knock();
  const show=v=>{ BUILD_LINE='build '+v; paintBuild(); };
  if(!/^https?:$/.test(location.protocol)){ show('local'); return; }
  fetch('build-info.json').then(r=>r.ok?r.json():null).then(j=>{
    show(j && j.version ? j.version+(j.builtAt?(' · '+j.builtAt.slice(0,10)):'') : 'unknown');
  }).catch(()=>show('unknown'));
}

/* ---------------------------- the testing door ----------------------------
   Every gate in the game — the realms after the first, the Deep, the Sanctum,
   the Arena — reads the same \`cleared\` map, so opening the whole game is one
   assignment. What takes the care is making it hard to arrive at by accident
   and impossible to be in without knowing: a player who stumbles into an open
   Sanctum at level one would think the game broken, and a save that has been
   opened this way must never be mistaken for a run someone actually made.
...
const Cheats = {
  ...
  DOORS:[
    ...
    {word:'arm the knight', ds:'the level, the gold, the gear and the materials',
     ...
     apply(g){
       ...
       g.mats=g.mats||{ore:0,essence:0,herb:0};
       if(g.mats.ore < 200){ g.mats.ore=200; ch=true; }
       if(g.mats.essence < 100){ g.mats.essence=100; ch=true; }
       if((g.mats.herb||0) < 150){ g.mats.herb=150; ch=true; }
       if((g.mats.page||0) < 150){ g.mats.page=150; ch=true; }
  ...
  knock(){
    const now=Date.now();
    this.taps = (now - this.at < this.GAP) ? this.taps+1 : 1;
    this.at = now;
    if(this.taps >= this.TAPS){ this.taps=0; this.open(); return; }
    // Silent until it is clearly deliberate, then it counts down out loud so
    // that whoever meant to find it knows they are on the way.
    if(this.taps >= 4) UI.toast((this.TAPS-this.taps)+' more');
  },
  ...
  open(){
    ...
    el.innerHTML=\`<div class="cxbox">
      ...
      <button class="btn ghost" onclick="Cheats.close()">Cancel</button>
    </div>\`;
    el.classList.add('on');
    document.getElementById('cheatGo').onclick=()=>{
      const said=document.getElementById('cheatIn').value;
      const res=this.say(said);
      if(!res.ok){ UI.toast(res.why); return; }
      this.close();
      UI.toast(res.note);
      UI.renderMap(); paintBuild();
    };
  },
  close(){ const el=document.getElementById('codeBox'); el.classList.remove('on'); el.innerHTML=''; },

  /* Spelling is forgiven — case, spaces and punctuation all go — because the
     word is a door rather than a puzzle, and a tester typing it on a phone
     keyboard should not be fighting autocorrect for entry. */
