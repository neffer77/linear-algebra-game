...
const RoomKinds = {
  monster: {
    ...
    build(rng, depth){
      return { kind:'monster', foe: WaveEngine.foe(depth, ARENA_WAVES) };
    },
...
const Dungeon = {
  ...
  descend(){
    ...
    this.run = { setting:'deep', seed, depth:0, unbanked:{ gold:0, xp:0 } };
    this.active = true;
  ...
  nextRoom(){
    ...
    R.unseed();
    this.cur = { kind, spec };
    const ctx = { run:this.run, depth, setting:this.run.setting,
                  rng:R, canDie:true, resume:null };
    kind.enter(spec, ctx, outcome => this.resolve(outcome));
  },
