...
const Game = {
  ...
  fresh(){
    return {
      ...
      mats:{ore:0, essence:0, herb:0},     // what the Deep and the Wilds give up; the Keep spends it
...
const RoomKinds = {
  ...
  forage: {
    build(rng, depth, set){
      ...
      const base = clamp(1 + Math.floor(depth/4), 1, 3);
      const qs=[];
  ...
  takeSighting(){
    ...
    for(let d=from; d<from+this.SIGHT_RANGE; d++){
      ...
      if(!pays && (r.name==='lock' || r.name==='seam' || r.name==='sigil'
                || r.name==='forage' || r.name==='hold')) pays=at;
      if(champ && pays) break;
    }
    const far=(n,what)=> n
