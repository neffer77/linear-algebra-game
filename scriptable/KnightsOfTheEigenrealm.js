...
const RoomKinds = {
  monster: {
    ...
    build(rng, depth, set){
      return { kind:'monster', foe: WaveEngine.foe(depth, (set||SETTINGS.deep).waves) };
    },
...
const Dungeon = {
  ...
  peek(depth){
    ...
    if(name==='monster') foe=WaveEngine.foe(depth, set.waves);
    if(name==='wager'){
  ...
  takeSighting(){
    ...
    for(let d=from; d<from+this.SIGHT_RANGE; d++){
      ...
      if(!pays && (r.name==='lock' || r.name==='seam' || r.name==='sigil' || r.name==='forage')) pays=at;
      if(champ && pays) break;
    }
    const far=(n,what)=> n
  ...
  descend(setting){
    ...
    this.run = { setting:key, seed, depth:from, paid:stake, from, wards:0,
  ...
  resume(){
    ...
    this.run = { setting:r.setting||'deep', seed:r.seed, depth:r.done||0,
                 paid:r.paid||0, from:r.from||0, wards:r.wards||0,
